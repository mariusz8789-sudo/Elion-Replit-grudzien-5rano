...
1. In Railway, create a new project from GitHub and select `mariusz8789-sudo/Elion-Replit-grudzien-5rano` with branch `railway-production-ready`. Railway will use the repository `Dockerfile` automatically; the fallback Node commands are `npm run build` and `npm start`.

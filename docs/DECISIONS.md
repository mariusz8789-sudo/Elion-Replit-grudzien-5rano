...
1. **Matrix background stays.** An "evidence field" replacement was built and then reverted the same day: the product owner keeps the Matrix and wants a *premium* version of it designed first (product palette, scientific glyphs, depth, readability behind panels), not a removal. `components/liveMatrix/*` is the original digital-rain engine, unchanged.
...
**Why.** The audit of the visual layer found a verdict that was shown as text while the mechanism behind it (conjuncts, G2 separation, gate, provenance, determinism) stayed invisible. Investors and reviewers must be able to *see* why a candidate won or why none did.
...
## D-118 — Genesis as a product: one design system, a layperson's menu, Start, Worlds, Matrix Premium (no scientific change)

**Decision.** The product owner's direction: keep the Matrix as brand DNA (premium, not removed), make the whole UI/menu beautiful and simple for a layperson first, build no new modules, and use the existing worlds (City 3D, Virtual Lab, Molecule Lab, Discovery Hall). Implemented as:

1. **Design system v4** (`styles.css` `:root` + the appended override layer): deeper navy-black surfaces that still go up in lightness, glass chrome, one role per colour — cyan = interactive/data, gold = decision/needs-a-human only, green = pass/real, red = refuse/veto, violet = visualisation/model — one button system (primary filled cyan · secondary glass · tiny), one card surface, sans eyebrows, fields lighter than the page (the console's white textarea is gone).
2. **Menu in a layperson's words** (`core/navigation.ts`): Start · Zapytaj · Odkrycia · Światy 3D · Dowody i pamięć · Ustawienia, each with a one-line description; the former ten entries moved into "Wszystkie moduły". Every hash still resolves (navigation ↔ router test).
3. **Chrome**: sidebar with the Genesis mark and descriptions; a top bar on every route (Start included) with the route name without emoji and an "Zapytaj Genesis…" field that opens the one Science Chat.
4. **Start** (`StartHero.tsx`): question box first, three doors (ask / see a discovery / enter a 3D world), a status strip of real reads (Science Memory, worlds, labs, backend). Everything Home used to shout stays behind one disclosure — nothing deleted.
5. **Worlds hub** (`#/worlds`, `WorldsHubScreen.tsx`): only routes that exist, each with its research purpose and explicit REAL / WIZUALIZACJA lines.
6. **World chrome** (`WorldChrome.tsx`) on City 3D, the Virtual Lab, Molecule World and Discovery Hall: glyph · domain · title · purpose · epistemic badges · live KPIs · the world's actions. The lab collapses to one column on phones; Molecule World's two floating panels stack instead of overlapping.
7. **Matrix Premium** (`liveMatrix/matrixEngine.ts`, API unchanged): the rain keeps its identity but is slower and sparser, uses scientific glyphs (hex, Greek, calculus, units), the navy/cyan/cool-white palette with the classic green on roughly one column head in eight, gold only at ATTENTION; the layer is edge-masked and dims on phones.
8. **The answer first** in the Research Console (`RunVerdictHero.tsx`): verdict, winner or blocker, gate outcome, conjunct ticks, four KPIs and jump links above the timeline; sandbox panels behind one disclosure; Winner Gate ports meet the boxes; figures scroll sideways on phones instead of shrinking their labels.
...
## D-121 — Cryptographic audit trail: a SHA-256 seal on every verdict, chained into a ledger (no scientific change)

**Decision.** Every finished run of the LOWER-HARM pipeline now carries `auditSeal` (`core/audit/cryptoAudit.ts`): a SHA-256 digest over a deterministic snapshot of what the run decided — scenario, mode, verdict, the internal audit and recipe fingerprints, the WinnerRecord (winner id, name, record fingerprint, gate outcome, gate fingerprint) or the exact NO_WINNER blocker, the three Winner Gate conjuncts with held/failed, the custody hash of the pinned dataset, and every stage's fingerprint. Seals chain: each one commits to the previous digest (`previousSha256`, `chainIndex`), so `verifyAuditChain` detects a changed byte in any link and a reordered, dropped or forged link.

- **Determinism is the contract.** The sealed payload holds no timestamp and nothing the run did not produce; `sealedAt` is metadata outside the hash. The same run seals to the same digest on any machine, so replay MATCH now also implies seal equality (the emit script refuses to write if the two seals differ).
- **Alongside, not instead.** The internal FNV fingerprints (`core/events/hash.ts`) stay exactly what they were — every pinned fingerprint (audit 6615057e / recipe 7ddcabe9 / record b6207e9c) and every replay comparison is untouched. The seal uses the repo's own `canonicalJson` and the same Web Crypto SHA-256 as the evidence custody digest (`evidenceCrypto.ts`), so it runs in the browser and in Node without a `node:crypto` import.
- **Committed trail.** `npm run winner-record:emit` now also writes `artifacts/lower-harm/audit-seal.json` (the seal of the canonical verdict) and `artifacts/lower-harm/audit-chain.json` (the ledger of every emitted verdict; a re-emit of an unchanged verdict re-seals the same link, a changed verdict appends a link that commits to the previous one; a broken chain is never extended). `npm run audit:verify` recomputes everything read-only and cross-checks the seal against `winner-record.json` and `replay-verification.json`; the D-116 lock test additionally checks that a live PRODUCTION run projects to exactly the committed snapshot.
- **In the product.** The verdict hero shows the seal (first 16 hex, full digest on hover) and the state of the person's own ledger: `core/audit/auditLedger.ts` appends each run's seal to a localStorage hash chain and verifies the whole chain after every append ("łańcuch audytu: n pieczęci · integralność OK", or NARUSZONY with the reason — never silently reset).
...
**Addendum (same day).** The Omni-Ingestion adapters (official APIs only, robots-aware web reader, legal-gate registry, propose-only hand-off) were added under `packages/core/src/knowledge/ingestion` with their tests. They are not wired to Science Chat: that needs a backend endpoint, a persistent ledger and a key policy, which is a product decision, not a patch.

## D-123 — Native System Orchestrator + Hybrid Quantum Bridge + GENESIS 2040 visual layers (no scientific change)

**Decision.** Three additions landed on this branch in one night (owner's brief of 2026-09-18: "integrate the native engine and the quantum bridge, the UI must look like 2040, not 1990"):

1. **Native System Orchestrator** (`packages/core/src/engine/native`). The delivered module was audited before integration; it had 17 defects (an inline `UNKNOWN_KIND` promise that never resolved — its own test timed out —, dead workers left in the pool with no `exit` handler, prototype-chain kernel lookup so `submit('constructor', …)` ran `Object(x)`, an stderr pipe that was never read (deadlock above 64 KiB), unbounded subprocess output and no timeout, `drain()` never released by `shutdown()`, tasks accepted after shutdown, workers never `unref`'d, unvalidated `matrixMul`). All fixed; `submit()` always returns a promise; a shared `workerProtocol.ts` serves inline mode and every worker entry; thread mode is bundled in-test with esbuild and verified for fault isolation, worker-death recovery and no dangling handles (31 tests). `streamSubprocess` stays an internal primitive with an executable allow-list and `shell:false`; it must never be exposed on an HTTP endpoint.
2. **Hybrid Quantum Bridge** (`packages/core/src/engine/quantum`, `packages/backend/src/quantumApi.mjs`, Science Chat `/quantum`). The "delivered Quantum Bridge" files were never received (two directives, zero files), so this is an in-house implementation of the owner's spec: OpenQASM 3.0 generator and parser (h x y z s t rx ry rz cx cz swap barrier measure), a deterministic statevector simulator up to 16 qubits (seeded sampling, `sha256(qasm + ':' + seed)`, no `Math.random`), a cloud QPU REST adapter (Bearer key from `QPU_API_KEY`, endpoint `QPU_API_URL`, never called without both), and `QpuOrchestrator` with a fallback gate (`NO_CLOUD_PROVIDER` / `NO_API_KEY` / `CLOUD_UNAVAILABLE` → local simulator). The backend runs local simulations under the Native Orchestrator's worker-thread pool (`WorkerComputeSink`), so a 16-qubit job never blocks the event loop. **Epistemic rule:** a local result is `executedOn: LOCAL_SIMULATOR`, `label: MODEL_ESTIMATE` and the chat says so ("NIE pomiar"); only a configured hardware provider yields `HARDWARE_MEASUREMENT`. Nothing here feeds the Winner Gate.
3. **GENESIS 2040 visual layers** (`styles-2040.css` shell, `styles-2040-screens.css` screens, `GenesisHoloBackdrop`, `components/holo/*`). Glass HUD chrome, luminous gradient borders, corner brackets, neon controls and filled range sliders, aurora and perspective-grid background, a Three.js holographic backdrop and per-world procedural previews. Every decorative element is `aria-hidden` and labelled "PODGLĄD PROCEDURALNY" where it could be mistaken for data; no number on screen was added by the restyle. Matrix rain identity kept. All 3D layers pause under `prefers-reduced-motion`, when the tab is hidden and on heavy-3D routes.

**Rejected the same night.** A re-emitted "Gemini" package of nine engine files whose names all already exist here in richer, tested versions (audit table in `docs/GENESIS_STATE_AUDIT_2026-09-18.md` §5.1); one of them implemented "Hadamard" as a global 1/√2 rescale. Its two Playwright specs targeted commands and selectors that do not exist; they were mapped onto the real product as `scripts/city-hud-e2e.mjs` (`npm run e2e:city-hud`, 6/6).

**Why.** Compute that runs in threads is a capability only when its failure modes are proven; a quantum result is honest only when it says what executed it; and a 2040 look must not smuggle numbers into the frame. Preregistration, thresholds, weights, Winner Gate, fingerprints and the D-121 seal chain are untouched.

**Addendum (2026-09-18, pre-deploy audit).** The owner's checklist asked for the 5D manifold engines to be connected to the backend and free of constants. Delivered drafts were rejected (hard-coded telemetry); in their place `packages/core/src/engine/manifold` implements real discrete differential geometry of a path in R⁵ (Gram-matrix metric, polyline curvature, exact segment–segment self-intersection test, the Tartaria SDF evaluated on the CPU with the same formula the GPU raymarches) with SHA-256 provenance, labelled `GEOMETRIC_MODEL`, and `GenesisEnterpriseCore` whose telemetry is sampled from the OS through `SystemResourceBridge`. Backend: `POST /api/manifold/evaluate`, `GET /api/system/telemetry` (label `MEASURED`). The 2040 HUD posts the backdrop camera's real flight path and shows the computed curvature/stability/SDF occupancy next to measured CPU/memory/load; with no backend it shows nothing. Nothing here touches Tor A. CI: the verify job now installs the pinned RDKit runtime and has a 30-minute limit (it had been red or cancelled on every run since 2026-09-16 for those two reasons alone).

## D-124 — Single-kernel policy (KernelProviderRegistry) + CLOCKWORK, the first B2G office module (no scientific change)

**Decision.** (1) Genesis has ONE orchestrator kernel: `core/agent/cyberReasoningKernel.ts`, already behind `#/cyber`. It binds itself to `packages/core/src/mythos/KernelProviderRegistry.ts` at load; any other module that tries to bind as a kernel throws `KERNEL_ALREADY_BOUND` (tested). The Mythos / Post-Mythos engines (CICADA, ZTSE, Action-Gate, PQC) are analysis PROVIDERS resolved by capability through that registry, never a second kernel; `#/matrix` is GPU visualisation only (rain, hypergraph, HUD) and binds nothing. Registered by default: ZTSE (needs no configuration) and CLOCKWORK. CICADA needs real baselines and Action-Gate a real signature verifier, so their providers are registered only by the owner of that configuration — never with invented baselines or a stub verifier. The `staging/qwen-cyber-foundation-unreviewed` engine (a second kernel with its own toy target) stays out for the same reason; the repo's module-reachability test enforces it.

(2) **CLOCKWORK** (`packages/core/src/mythos/clockwork/ClockworkEngine.ts`, capability `deadline-monitoring`): statutory deadline arithmetic for a Polish office — KPA art. 35 §3 (1 / 2 months, appeals), art. 57 §1/§3/§4 (day and month counting, Saturday/holiday shift to the next working day), UDIP art. 13 ust. 1/2 (14 days, extension to two months) — over a computed Polish public-holiday calendar (Easter by Meeus, Corpus Christi, 24 December from 2025). It classifies cases ON_TRACK / DUE_SOON / OVERDUE / CLOSED with working days left and an escalation level, and drafts the notices (art. 36 §1 KPA, art. 13 ust. 2 UDIP, art. 37 escalation note) as `DRAFT_FOR_HUMAN_APPROVAL` with the reason fields left for the clerk. Every assessment and every draft is anchored in the EvidenceLedger (record ids returned; chain verifies). No wall clock in the arithmetic (`today` is an input), no randomness, no send/execute; label `DETERMINISTIC_CALENDAR_MODEL`. 26 tests (calendar law, classification, drafts, extensions, ledger anchoring, determinism, malformed dates, registry, iron rules).

**Why.** Simulations, worlds and the Matrix are the showroom; an office buys what takes hours off one clerk's desk, and the cheapest hour to remove with the lowest legal risk is the missed statutory date. The single-kernel rule exists so that the analytical modules can multiply without a second decision path ever appearing: one kernel, many providers, and the human signs.

**Not claimed.** This is calendar arithmetic over statute text, not legal advice: the office's lawyer owns the interpretation (e.g. whether UDIP terms follow art. 57 §4 KPA in their practice), and every draft is a draft until a person approves it.

## D-125 — Genesis Collider + Quantum Lab FPV as kernel providers (toy-normalised models, labelled; no scientific change)

**Decision.** Two engines delivered by the architect land as PROVIDERS of the single kernel (D-124), never as routes with their own decision logic: `packages/core/src/collider/QuantumColliderEngine.ts` (capability `particle-collision-sim`) and `packages/core/src/lab/ThermodynamicLabEngine.ts` (`thermodynamic-reaction-sim`). Both are wrapped in `KernelProviderRegistry` so that every generated event and every mix is committed to the kernel's EvidenceLedger by the engine's own `commitToLedger()` before the UI sees it; the provider result carries the ledger `contentHash` and a label (`TOY_MC_MODEL`, `THERMODYNAMIC_MODEL`). Views: `#/collider` (detector chamber: seed, event index, pT and process filters, bent tracks in 3.8 T, calorimeter hits, full readout with event hash and ledger hash) and `#/lab-fpv` (first-person bench: reagents in mol, ignition, T₀; ΔH/ΔS/ΔG from the tabulated ΔfH°/S°/Cp, limiting reagent, adiabatic temperature, visible outcome; state file = the engine result verbatim + ledger hash; webm recording of the real canvas via MediaRecorder). Delivered files kept as written except: `add4` exported (unused-local under the frontend's strict tsc), `REACTIONS` cast to `ReactionDef[]` (TS2322 on the frozen literal). The shell backdrop is suppressed on both routes (they own their WebGL).

**Honest boundaries (as the package itself states).** Cross sections, shower and hadronisation are toy-normalised (Pythia-inspired) models, not PDG precision; masses and thermodynamic data are standard tabulated values (PDG 2022 / NIST-CRC rounded). The label is on screen and in the ledger claim (`claimType: 'model'`). Nothing here feeds the Winner Gate.

**Tests.** `collider.test.ts` (determinism and hash consistency, acceptance cuts, Z→μμ mass reconstruction within the generator width, running coupling, ledger dedupe, provider through the registry, iron rules) and `lab.test.ts` (Hess arithmetic, endothermic/spontaneity vs T, stoichiometry and limiting reagent, outcomes incl. explosion and crystallisation, no-reaction path, determinism per seed, ledger, provider, iron rules); frontend `colliderChamber` / `labFpvView` (provider boundary, state file, static render).

## D-126 — CERN complex (5D route) as kernel providers + the visual fidelity loop (labelled models; no scientific change)

**Decision.** The two delivered CERN packages land as PROVIDERS of the single kernel (D-124) behind one full-viewport route `#/cern-complex` (`packages/frontend/src/components/CernComplexView.tsx`): `blackHoleProvider` (`micro-blackhole-sim`, `BlackHoleEventHorizonEngine`: Schwarzschild radius, Hawking temperature and lifetime, deterministic Hawking spectrum, evaporation timeline), `materialsProvider` (`crystal-synthesis-sim`, `MaterialsDiscoveryEngine`: lattice choice, cell constant, density, bulk modulus, conductivity, DOS, formation energy as documented ESTIMATES, not DFT) and `computeColliderProvider` (`collision-batch`, `ComputeColliderEngine`: a label-seeded batch of collider events with instancing track attributes, anchored as one ledger record). Every attempt, synthesis and batch is committed to the kernel's EvidenceLedger by the engine's own `commitToLedger()` / `commitBatch()` before the HUD shows it; the provider result carries the ledger `contentHash` and the engine's own epistemic label. Labels are not softened: a 4D micro black hole needs the Planck energy and is a HYPOTHESIS; the TeV-scale ADD scenario is SPECULATIVE with reason `ADD_TEV_SCENARIO_SPECULATIVE_NO_EVIDENCE`; 13 TeV alone is `BELOW_THRESHOLD` / `NOT_FORMED`; materials carry `EMPIRICAL_ESTIMATE_MODEL`; the batch `TOY_MC_MODEL`.

**The picture.** Delivered GPU modules kept as written (`LabComplexGpu`, `TunnelWalkthroughGpu`, `EventHorizonShader`, `Cern5dPostProcessing`): hub, doors, bench, consoles with screen shaders, refraction glass, LHC-like ring segment with dipoles and cryo lines; the event horizon (screen-space lensing, accretion disk with Doppler beaming, photon ring, shadow) rendered to a 3:1 texture that the glass refracts and that fills the hall beyond it; the delivered post pipeline (SSAO, SSR on the floor and the glass found in the scene, bloom, volumetric scattering, lens/glass pass with chromatic split, output). The host adds what the modules leave to it: PBR dressing (instanced pillars, pipe runs, racks with emissive LEDs, ceiling strips with additive volumetric cones), the lattice of a synthesised crystal on the bench (one instance per engine site), the collision hologram, camera poses per mode (WALK / GLASS / CONSOLE / TUNNEL on keys 1–4, Q/E/R actions), a planar reflector as the fallback when SSR is unavailable, and a measured frame budget that steps the pipeline down (`cinematic` → `balanced` → `performance`) from the composer's own frame cost, never from a guess about the GPU. FPS and the FPV position on the HUD are measured; there is no beam-status or other invented telemetry.

**Eyes and hands.** `packages/e2e/src/cern-complex.e2e.spec.ts` drives the four modes on the production server, writes `artifacts/{walk-5d,glass-refraction,console-pbr,tunnel-volumetric}.png` and runs the delivered `VisualFidelityHarness` (a dependency-free PNG decoder + documented rejection rules: flat, black, Matrix-green, wireframe-dominated, colour-poor). The loop was run by hand as well: the first render was blown out (exposure 1.15, 22 cd ceiling lights, emissive 3.2, stacked cones on the centre line) and was corrected in three passes (exposure 0.82 / 0.72 in the tunnel, 8 cd lights, emissive 1.2, cones off the centre line, wall washes, a headlamp inside the ring, GLASS/CONSOLE poses pulled back so the frame and the three consoles read, TUNNEL start on the walkway side of the magnet string).

**Delivered-code corrections (all type- or lint-required or plain defects, each visible in the diff).** `EventHorizonUniforms` gets an index signature (TS2322 against `ShaderMaterial.uniforms`); `ArcCurve` gets a public constructor (protected `Curve` constructor); unused `T`, `rng` and the `seed` parameters renamed `_seed` (noUnusedLocals/Parameters); `ComputeColliderEngine` imports the collider engine from `../collider/` (delivered path did not exist) and reads `f.pdg` where `f.pgId` was a typo; `SSRPass` gets the `groundReflector: null` its typings require; `evaporationTimeline` drops a dead `M = 0` assignment (lint); `VisualFidelityHarness.luma` no longer divides by 255 a second time (the delivered version normalised twice, so every frame — including a real render — was `FLAT_IMAGE`; unit test added). The delivered `packages/ui/src/cern/CernComplexView.tsx` was NOT integrated as the route: it binds a second kernel id, registers ad-hoc providers on a private ledger (not the kernel's), shows a `BEAM: RAMP/STABLE/FLAT TOP` label cycling on the clock (invented status), and duplicates `Cern5dPipeline`; its user-facing features (badges, hash strip, FPV/FPS, Q/E/R, SSR selects, lens on formation) are carried into the frontend view. The first, superseded batch (`Cern5dPipeline.ts`, `visualInspection.ts`, the ui-package e2e) was not saved.

**Tests.** `cern.test.ts` (textbook formulas against a solar mass, threshold/regime/label logic, determinism per seed, Hawking spectrum weights, evaporation monotonicity, lattice selection and cell counts, ledger anchoring/dedupe/verify for both engines, the three providers through the registry, `KERNEL_ALREADY_BOUND`, iron rules), `VisualFidelityHarness.test.ts` (pixel-exact decode, rejections, acceptance), frontend `cernComplexView` (provider boundary, lattice instances, static render without WebGL), navigation and reachability suites.

## D-127 — Human Biology Lab (V3 pack) on the canonical Scientific Worlds stack; cognitive core (V4) behind real approval; flagship visual pack reviewed

**Decision.** The delivered `GENESIS_Scientific_Worlds_Execution_Pack_V3_Human_Biology_Lab` is saved as written under `packages/frontend/src/core/scientificWorlds/humanLab/` (anatomy atlas, anatomy view, visual modes, physiology, neuro lab, Hyperscope, virtual microscope, histology, imaging centre, ORPHEUS, specimen registry, inventory, protocols, biosafety, stations, realism, lab environment, command router, hash; plus the pack's own `epistemic.ts` and the three `EvidenceSink` types from its `contracts.ts`). Only import paths changed (`../execution/*` → local) and one unused type import was dropped (`LabZoneId`, noUnusedLocals). It runs on the stack from D-126/921ee18a, not beside it: `biologyLabWorld.ts` turns `createGenesisBiologyLabScene()` into canonical `LabStation`s with the pack's `station:*` ids and positions verbatim (the host adds facing, standoff, footprint, keywords and which experiment a console runs); `biologyCommands.ts` lets the pack's `parseBiologyLabCommand` classify each clause and maps its kinds onto the WorldCommand bus (`OPEN_TWIN` → walk to the anatomy table + interact, `FOCUS_ANATOMY` → interact with `{focus, mode}`, `CAPTURE_HYPERSCOPE` → run `hyperscope-capture` with the magnification, `RUN_ORPHEUS` → run `orpheus-scan`, `SHOW_EVIDENCE` → inspect with provenance; the "5×" sign is normalised to the `5x` the router reads); `biologyRunners.ts` seals every pack instrument as ONE canonical `ExperimentSession` with the pack's own epistemic label (`MODEL` / `SIMULATION`; the pack's `RECONSTRUCTION` is a model-derived view and maps to `MODEL`, the instrument label is kept in the outputs) and the pack's `EvidenceSink` implemented by the kernel's EvidenceLedger — the wall-clock `sourceTimestamp` the pack stamps is dropped so a replay reproduces the ledger hash (MATCH). The action machine gained `INTERACTION_SETTLED` (a view-only interaction returns the hands to ARRIVED) and an observation followed by more work closes as observed; the planner carries INTERACT parameters; the controller reports them once, so the screen applies the pack's `anatomyView` reducers and the scene shows the mode (`data-twin-mode`).

**The picture (graphics review of the packs).** The V2/V3 zips carry no screenshots and no binaries: asset slots are contracts. The repository's only human model (`mpfb-lod0.glb`) is `UNVERIFIED` in `assetGovernance` and no organ/instrument GLB exists, so the Human Digital Twin is an explicit PROXY: the Genesis character rig at the manifest's 1:1 height rendered as a luminous holographic body (emissive shell + a point cloud sampled from the rig's own vertices — the reference look), with one ellipsoid per ORGAN node at the atlas's positions and dimensions and the NEURO_REGIONS inside the brain; the visual modes (`buildVisualLayerInstruction`) drive visibility, translucency and tint; the HUD says `PROXY · MODEL`. The lab is built by `AgentLabScene3D` (one scene class, one pipeline, the same VISOR/SPECTATOR cameras) from the pack's description through two new kits: `biologyLabKit` (layered ceiling: slab, duct layer, coffer grid with a light trough per coffer; glass curtain walls with mullions and a lit corridor behind; the twin chamber with ribs, LED base, emitter, concentric overhead rings; six-axis manipulators; text signs at the pack's sign nodes; mezzanine gallery; holographic label panels; Kelvin → RGB for the pack's light temperatures) and `biologyStationKit` (anatomy light-table with the twin lying 1:1, Neuro console with a NEURO_REGIONS hologram, Hyperscope hero and a precision microscope, histology bench with slide rack/microtome/stains/cell dome, CT-like gantry with patient table, ORPHEUS sealed chamber with carousel, optical head, two manipulators and a diagnostic display, compute wall with LED racks, evidence wall, safety pedestal with PPE lockers). Screens draw only from sealed sessions (`drawBiologyArtifact`: vitals, signal bars, field of view with the seeded cell model or a hash-seeded tissue field, slide, atlas slices cut by the requested plane, ORPHEUS metric bars); the evidence wall lists sealed sessions only. Light budget: the forward renderer pays every point light per fragment, so the room keeps ≤ 12 point lights (pack key/fill/rim, two ceiling practicals, five hero bays, the chamber emitter) — the first render at 18 lights cost ~4 s/frame under software GL, 0.5 s after; the pack's key at 12 000 lm is mapped at 0.6× (the floor blew out at 1.4×). Post: bloom/AO as the physics lab; the realism profile's TAA, lens distortion, chromatic aberration, film grain and visor condensation are NOT implemented by `setupGraphicsPipeline` and are not claimed.

**Flagship visual rebuild pack (with two reference renders).** Reviewed against the references: integrated as the reference-driven upgrades above (hologram twin, chamber crown/ribs/rings, flanking manipulators, mezzanine, holo panels) and as `graphics/visualRealityGate.ts` (the pack's density gate — triangles, opaque/transparent meshes, renderer draw calls — in the kit convention; re-measured every 60 frames and exposed in the scene stats). NOT integrated as code: `labSetDressing.ts` (unit boxes; the Genesis kits are richer), `materials.ts` (transmissive glass reads as a white wall under software GL and duplicates the palette), `lighting.ts` (directional key over the pack's own light nodes), `createFlagshipVisualLayer.ts` (a parallel entry point; the entry point is `initBiology`), `humanAsset.ts` (a GLTF loader path — the repository already has `loadGltfCharacter`; with no licensed, approved human GLB the asset path is BLOCKED and the twin stays a labelled proxy, as the pack itself demands: "do not fake success"). Direct `import * as THREE from "three"` in the pack violates the graphics boundary rule, another reason the modules were not copied verbatim.

**Cognitive core (V4).** `GENESIS_AGI_COGNITIVE_BRAIN_ONLY` saved as written under `packages/core/src/cognitive/` (world model, memories, goals, planner, hypothesis engine, evidence reasoner, falsification, experiment planner, self-monitor, proposal gate, command policy, collaboration, null language model, adapters). One correction: `stableHash` used `JSON.stringify(input, Object.keys(input).sort())`, whose replacer array applies to every nesting level — nested keys absent from the top level were dropped (plans differing only in `steps[].actionType` hashed alike; `facts` arrays hashed as empty); replaced by canonical JSON with the same FNV-1a and output shape (test). The existing `GenesisAgisCognitiveCore` (supreme) stays as is. `cognitiveBridge.ts` binds the adapters to the canonical systems and adds what the pack leaves to the host: a `BIOLOGICAL`/`IRREVERSIBLE` command and every experiment proposal are dispatched only with an approval token a named human granted for that exact id (the pack's policy passed on the command's own `requiresApproval` flag); `WORLD_COMMAND` text goes through the same parser and planner as the chat bar and the plan is handed to the host; proposals run through `createExperimentSession` only at a station that runs an experiment; the core may append hypotheses/model notes to the kernel ledger and can never assert `REAL_OBSERVATION`/`VERIFIED_SOURCE`; world entities are derived from the stations and the twin atlas. Science Memory is NOT bound (its savers are typed per artefact; the generic `write(record)` has no honest target) — the core's own long-term memory is used and this is stated here. Nothing in UI or code calls this an AGI.

**Tests.** `scientificWorldsBiology` (pack ids preserved, every station reachable by A* from the spawn, the V3 acceptance sentence → 7 commands on the pack station ids, `25×`/`100x`/RTG/histology/imaging/neuro parsing, all six runners seal with ledger hashes, non-observation status and replay MATCH, Hyperscope mode by magnification, ORPHEUS determinism and biosafety `ACCESS_RESTRICTED`, the agent executing the whole acceptance plan with two interactions and two sessions), `scientificWorldsCognitive` (attach, WORLD_COMMAND → plan, approval gate grant/revoke, proposal → session → MATCH, ledger writes and the observation refusal, entity derivation), `packages/core/src/cognitive/cognitive.test.ts` (the delivered self-check ported, blocked/deferred cycles, gate, falsification, hash), and the Playwright flow `human biology lab` in `scientific-worlds.e2e.spec.ts` (the acceptance sentence end to end with fidelity-gated screenshots).

## D-128 — Autonomous Scientific Intelligence audit: what exists, what was added on top of it (no duplicates)

**Audit (three read-only sweeps, results verified against `moduleReachability`).** Already implemented and wired: the autonomous research loop (`discoveryLoop`, `inquiryLoop`, `discoveryOrchestrator`, `campaignOrchestrator`, `researchCampaign`, `researchChain`, backend `campaign/orchestrator.mjs`), hypothesis generation (`hypothesisLoop.generateCompetingHypotheses`, `mechanismGeneration`, `novelHypothesisGenerator`, `HypothesisTournament`, `beliefRevision`), falsification and preregistration (`preregisterHypotheses` / anti-HARKing anchors, `selfFalsificationBattery`, `falsifiedModelRegistry`, tautology / novelty / integrity / winner gates), experiment planning (`nextAction` selectors, `nextQuestion`, `differentiatingExperimentGenerator`, `scientificPlanner`, backend `nextExperiment.mjs`), simulation exploration (lookingGlass, `forkBranch`/`compareBranches`, `scenarioEngine`, `discoverySweep`, experimentFabric counterfactuals, `decisionMonteCarlo`, what-if), knowledge ingestion (`EvidenceLedger`, `OmniIngestionController` with the legal allow-list, `/api/knowledge/*`, document ingestion, the 23 public-source `knowledge/*.md` files behind `/api/ask`). Partial: contradiction detection (competing-model verdicts, MODELS_DIVERGE, a caller-supplied `unresolvedContradictions` list — nothing computed it), epistemic labelling (three vocabularies, two unbridged), curiosity (`nextQuestion` only from a finished run; `discoveryQuestions` is a closed catalog), molecular biology (prose knowledge files, `GenesisBioVirologyEngine` kinetics unreachable, organelles as enums). Missing: an Environmental Digital Detective (no module; DEFRA/AURN exists only as a CI fixture job; `causalInference` CAP-2 deliberately unwired).

**Added, each on an existing seam.**
- `packages/core/src/knowledge/contradictionHunter.ts` — `huntContradictions(records)`: NUMERIC_DISAGREEMENT (same `key=value` token, different values, different hosts), POLARITY_CONFLICT (near-identical wording, one negated), STATUS_CONFLICT (same claim, rejected vs not); deterministic ids and fingerprint; reports, never resolves; `unresolvedContradictionLabels` feeds `practicalCandidateGate`'s existing input.
- `packages/core/src/knowledge/curiosity.ts` — `generateCuriosityQuestions(records)`: RESOLVE_CONTRADICTION → INDEPENDENT_CONFIRMATION (single-host candidate/unverified claims) → MODEL_TO_OBSERVATION (numeric keys that exist only as model outputs), the codebase's lexicographic discipline (no scoring), every question citing its record ids and carrying INSUFFICIENT_EVIDENCE; `curiosityGoalProposals` → GOAL proposals (source RULE) that the delivered cognitive core's ProposalGate admits like any proposer's output.
- `packages/core/src/knowledge/molecularBiology.ts` — the standard genetic code (64 codons), `transcribe`, `translate` (first AUG to the first in-frame stop), `reverseComplement`, `gcContent`, `atpBudget` (glycolysis net 2 ATP + 2 NADH; aerobic ≈ 30–32 ATP with the historical 36 reported as historical; TEXTBOOK_ESTIMATE), `centralDogmaReport` (MODEL, content-hashed) and `commitCentralDogmaReport` on the ledger. Kernel provider `molecularBiologyProvider` (`central-dogma-model`) in `KernelProviderRegistry`, registered by the single kernel; the Human Biology Lab's compute wall runs it as the `central-dogma` experiment (a DNA run in the command is the input; otherwise a named default reference ORF), narrated with the textbook caveat.
- `packages/frontend/src/core/agent/environmentalDetective.ts` — `investigateEnvironmentalCase`: robust-z anomalies (median/MAD, |z| > 3.5), then the existing CAP-2 estimators (synthetic control with ≥ 2 controls, DiD with one, interrupted time series with none) and `testParallelPreTrends`; verdict EFFECT_SUGGESTED / NO_EFFECT_DETECTED / INSUFFICIENT_EVIDENCE with reasons; input provenance is declared (REAL_DATASET with a pinned URL, or SYNTHETIC) and a SYNTHETIC case can never yield more than INSUFFICIENT_EVIDENCE about the world; the detective never fetches. Kernel provider `environmental-detective`, anchored on the kernel ledger. `causalInference.ts` is therefore reached from the running app and its allowed-orphan entry is removed. No real AURN fixture is committed in the repository (the CI job only uploads an artifact), so no real-data verdict is claimed here.
- `cognitiveBridge`: `contradictions()`, `unresolvedContradictions()`, `curiosity(limit)`, `adoptCuriosityGoals(limit)` (idempotent by goal id); a goal that targets evidence records — not world entities — cycles to BLOCKED (`TARGET_NOT_IN_WORLD_MODEL`), which is the honest outcome, not a fabricated plan.
- `epistemicReliability`: `sessionStatusToCanonicalReliability` (Scientific Worlds / V3 labels) and `starScaleToCanonicalReliability` + `countFilledStars` (the `/api/ask` ★ scale) onto the one canonical rank; never an upgrade.

**Routed.** Science Chat gained `/dowody <pytanie>` (also "co wiemy o …", "jakie są dowody na …"): the `LaypersonAssistant` over the kernel ledger answers with the status of every source, refuses medical/legal/financial roles, and says "Nie wiem" on an empty match — the resolver only routes (`evidenceAnswer` action), it never words an answer; and `/ciekawość [n]` (also "jakie pytania warto zadać", "co warto zbadać", "luki w dowodach"): the curiosity questions from the kernel ledger's gaps, each with its record ids, or an explicit "no gaps" line.

**Not done, stated.** `sovereignTruthAnswer` remains unrouted (a product decision on which surface answers with it); the EvidenceLedger stays in-memory; no organism-specific biology (no splicing, no folding solver); the detective has no UI and no real dataset in the tree; hazard registry adapters remain NOT_IMPLEMENTED.

**Tests.** `packages/core/src/knowledge/knowledgeLayers.test.ts` (hunter determinism and the three detectors, curiosity ranking/citations/proposals, genetic code/transcription/translation/ATP/ledger), `environmentalDetective.test.ts` (synthetic step measured yet INSUFFICIENT_EVIDENCE for SYNTHETIC input, EFFECT_SUGGESTED on a pinned dataset, NO_EFFECT, estimator selection, refusals, anomalies, kernel provider + ledger), `scientificWorldsCognitive` (+ contradictions → curiosity → gated goals → honest BLOCKED), `scientificWorldsBiology` (+ central dogma at the compute wall, replay MATCH, narration), `epistemicReliability` (+ D-128 mappings).

## D-129 — Guide Voice "Museum Calm" and the multilingual Human Explorer packs on the existing engines (no scientific change)

**Audit of the two delivered packs** (`GENESIS_GUIDE_VOICE_MUSEUM_CALM`, `GENESIS_ULTIMATE_SCIENCE_HUMAN_EDUCATION_MULTILINGUAL`, plus the Human Explorer UI reference image). Both describe systems the repository already has: a voice engine with a provider chain (`core/guide/voiceEngine.ts`, pre-rendered audio → browser SpeechSynthesis, no paid TTS key), a narration model with `GuideLang`, an i18n module with the no-auto-translation policy, the V3 anatomy atlas and visual modes, the Human Biology Lab stations and runners, the cognitive core with its approval gate, the EvidenceLedger and `LaypersonAssistant`, Science Chat. Neither pack is copied as a system; only the deltas that had no canonical equivalent were added, each on the existing seam.

**REUSED (no new code).** Voice engine and its settings/captions path; `guideRuntime.getVoiceEngine`; narration lines from sealed sessions; `t()`/`setLocale` and the subscribe path; the atlas ORGAN nodes and `HumanDigitalTwinManifest`; `histology-slide`, `hyperscope-capture`, `central-dogma` experiments; `WorldCommand` bus + `parseBiologyWorldCommands`; `LaypersonAssistant`, `huntContradictions`, `generateCuriosityQuestions`; `resolveCommand`'s `/dowody` route.

**EXTENDED.**
- `core/i18n.ts`: `Locale` is now `'pl' | 'en' | 'es' | 'ar'`; the `es`/`ar` dictionaries hold only the pack's own explorer vocabulary verbatim (no guessed words — a missing key falls back to Polish, a key missing everywhere stays visible); `localeDirection` and `setLocale` set `document.documentElement.dir/lang` (RTL for Arabic; ids, hashes and symbols untouched); `t(key, locale?)` for per-call locale.
- `core/scientificWorlds/humanExplorer.ts` (NEW file on existing seams): the BODY → ORGAN → TISSUE → CELL → ORGANELLE → MOLECULE ladder bound to atlas organ ids and the canonical experiments (histology at TISSUE, Hyperscope 100×/500× at CELL/ORGANELLE, central dogma at MOLECULE); every level carries the pack's evidence mode and `canClaimDirectObservation` is true only for REAL_IMAGE/REAL_DATASET; `parseExplorerZoom` (pl/en/es/ar) → `explorerCommands` on the WorldCommand bus; `biologyCommands.ts` tries the zoom parser before the pack router. The click-through UI of the reference image is NOT built (no organ GLBs, no histology/microscopy datasets exist in the tree — REQUIRED FROM USER; the ladder today is driven through the command bar and the sealed sessions).
- `core/guide/museumCalm.ts` (NEW): `museumCalmSettings` (rate ≤ 0.84, volume ≤ 0.78, captions forced on, language/enabled kept), `chunkForSpeech` (one idea per utterance, sentence then word boundaries), `epistemicStatusLine` (pl/en; unknown status → null, never invented), `museumUtterances`; `ScientificWorldsScreen` speaks narration through it with the sealed session's epistemic status first.
- `packages/core/src/knowledge/truthResponse.ts` (NEW): the pack's answer shape (status, evidence, contradictions, missing evidence, next tests, provenance ids, disclaimer, fingerprint) composed from `LaypersonAssistant` + `huntContradictions` + `generateCuriosityQuestions`; a boundary question with nothing in the ledger ("czy żyjemy w symulacji?") is INSUFFICIENT_EVIDENCE with empty sections rendered explicitly. Science Chat's `/dowody` now renders this shape (`renderTruthResponsePl`).
- `packages/core/src/knowledge/molecularSeed.ts` (NEW): the pack's DNA/RNA/ATP curriculum seeds and research prompts as data (subtopics, starter questions, evidence required) — curricula, not facts; `seedTopicFor` by keyword overlap, null when nothing matches.

**BLOCKED / NOT NOW (stated, with reasons).** School curriculum and quiz engine (a product outside the science stack; no owner decision); genealogy (personal data, privacy, no consent model); government contracts material (not code); the knowledge auto-learning scheduler and a YouTube provider (would be a second ingestion scheduler beside `OmniIngestionController`; YouTube terms); environmental candidate-site scoring / truck-risk models (would produce accusations from models — D-128's detective stays at INSUFFICIENT_EVIDENCE for anything not a pinned dataset); paid neural TTS voices (no keys by policy).

**Tests.** `humanExplorer.test.ts` (i18n es/ar + fallback + visible key + RTL flip; ladder on atlas ids; path/experiments/magnifications; zoom parsing in four languages; canonical commands with deterministic ids; bridge routing; museum-calm settings, chunking, status lines), `packages/core/src/knowledge/truthResponse.test.ts` (INSUFFICIENT_EVIDENCE on the boundary question, UNVERIFIED with provenance and a cross-host contradiction, seeds). Existing Science Chat, biology and reachability suites unchanged and green.

## D-130 — Gap Closure: ledger persistence, the autonomous curiosity cycle, Molecular Biology 2.0, the Environmental Detective's case graph, Human Explorer UI, and the flagship agentic runtime (no scientific change)

**Audit of the three delivered packs** (`GENESIS_Gap_Closure_Research_Memory_Biology_Detective_HumanExplorer_CLAUDE`, its V2 "Mirror Portals Cosmos" expansion, and `GENESIS_Agentic_Science_World_OS_E2E`, a superset of both). All three state their own reference adapters are test doubles and instruct mapping their ports to canonical services; none is copied as a system. Every module below sits on an existing seam from D-124–D-129; no second Cognitive Core, Science Memory, Evidence Ledger, Experiment Fabric, Scenario Engine, World Model or Voice Engine was created.

**GC-1 — Ledger persistence (PRIORYTET 1).** `packages/core/src/knowledge/EvidenceLedger.ts` gained `toSnapshot()`/`EvidenceLedger.fromSnapshot()` (a plain-JSON snapshot of entries, records, proposals, active order and version) and an `onAppend` hook; a restored snapshot's hash chain is verified before use — a broken or tampered one is `REJECTED`, the caller falls back to a fresh in-memory ledger, and the bad snapshot is left on disk for inspection (`ledgerPersistence.ts`: `restoreLedger`, `attachLedgerPersistence`, `openPersistentLedger`). Bound to the two existing stores rather than a new database: the backend's knowledge API (`packages/backend/src/knowledgeApi.mjs`) now opens a file-backed snapshot beside `genesis.db` (`GENESIS_LEDGER_PATH`, default `<data dir>/evidence-ledger.json`, atomic rename on save) at boot (`openKnowledgeLedgerPersistence`, called from `server.mjs`; status surfaced in `/api/health` and the startup log); the frontend's kernel ledger (`kernelLedger` in `cyberReasoningKernel.ts`) now opens through the existing `core/storage` localStorage wrapper (`core/knowledge/ledgerStore.ts`, key `kernel-evidence-ledger/v1`). Proposals, published records, contradictions and curiosity questions survive a restart because they are derived from the same restored records; the append-only, hash-chained shape is unchanged.

**GC-2 — Autonomous curiosity cycle + Science Memory ↔ Cognitive Core (PRIORYTET 2/3).** `core/scientificWorlds/curiosityCycle.ts` runs the full loop on existing modules: `generateCuriosityQuestions` (ledger gaps, D-128) → ledger retrieval + `LaypersonAssistant` (verify) → a hypothesis pair from `beliefRevision.createHypothesis` on the disputed numeric key → the world's own instrument that produces that key (`instrumentsFor`, found by one deterministic probe run per experiment on a scratch ledger, never the kernel one) → `createExperimentSession` through the bound runner, gated by the same human-approval token every proposal needs → `updateConfidence` (log-odds) → `core.ingestObservation` → a typed Science Memory record → the next un-investigated question. A question with no numeric key, or a key no instrument in the world produces, ends honestly (`NO_NUMERIC_KEY` / `NO_INSTRUMENT_FOR_KEY`) and is still remembered; nothing here ever fetches a network source — an `INDEPENDENT_CONFIRMATION` question names the source search it cannot perform (`sourceSearch.ingestionRequest`) and leaves it to the existing `/api/knowledge/ingest` + human publisher. The Science Memory bridge D-127 left unbound (`ScienceMemoryAdapter` needs both `write` and `read`) is now bound: `scienceMemory.ts` gained a typed `SavedCuriosityCycle` record (question, hypotheses with prior/posterior confidence, the experiment's session id/hashes, terminal reason) with its own guard, saver (`saveCuriosityCycleToMemory`) and reader (`listCuriosityCycles`); `core/scientificWorlds/scienceMemoryPort.ts` implements the port over that saver and refuses any other record shape (`SCIENCE_MEMORY_REJECTS_UNTYPED_RECORD`); `cognitiveBridge.ts`'s `CognitiveWorldBinding` takes an optional `memory` and wires it into `createCognitiveCoreAdapters`'s `memoryWrite`/`memoryRead`.

**GC-3 — Molecular Biology 2.0 (PRIORYTET 5).** Extended `packages/core/src/knowledge/molecularBiology.ts` (not a second engine) with seven labelled, deterministic mechanism functions and one dispatcher: `dnaReplication` (semiconservative, the textbook fidelity ladder — polymerase selectivity × proofreading × mismatch repair ≈ 10⁻⁹/base), `dnaRepair` (damage class → textbook pathway, double-strand breaks route to HR only with a sister chromatid else error-prone NHEJ), `rnaProcessing` (cap/splice/poly-A bookkeeping, a combinatorial exon-skipping bound), `proteinFoldingAbstraction` (declared toy two-state free energy → Boltzmann folded fraction — explicitly not molecular dynamics), `electronTransportChain` (10 H⁺/NADH, 6 H⁺/FADH₂, ≈3.67 H⁺/ATP synthase stoichiometry, coupling efficiency and leak as hooks), `atpBookkeeping` (Atkinson energy charge, 0.8–0.95 healthy band), `metabolicFlux` (`METABOLIC_NETWORK`: glycolysis/fermentation/pyruvate oxidation/citric acid cycle/oxidative phosphorylation as a lumped steady-state network; anaerobic 2 ATP/glucose, aerobic ≈30–32). The kernel provider `molecularBiologyProvider` now serves both the existing central-dogma request and `{ kind, input }` mechanism requests on the same ledger.

**GC-4 — Environmental Detective case graph (PRIORYTET 6).** `core/agent/environmentalCaseGraph.ts` builds the explainable graph over a D-128 `DetectiveReport`: actor/site/event/source nodes (each with declared source ids and a confidence derived only from known sources), typed edges, GIS signals (source, timestamp, spatial reference in EPSG:4326, detection method, evidence grade A/B/C/SYNTHETIC, uncertainty), counter-explanations and next actions. Status is `CANDIDATE_SITE_FOR_REVIEW` only when the series is a pinned `REAL_DATASET`, the detective's own verdict is `EFFECT_SUGGESTED`, at least one non-synthetic sourced signal exists, and both counter-explanations and next actions are non-empty — any missing condition is `INSUFFICIENT_EVIDENCE` with the reason stated; a synthetic series or synthetic-only signals can never produce a candidate. No field names or scores a responsible party; the fixed disclaimer states the graph is a review aid, never a finding. GeoJSON (`FeatureCollection`, EPSG:4326) is derived for signals and sites with spatial references. Wired as the detective provider's second capability (`environmental-case-graph`, request shape `{ caseGraph }`), anchored on the same kernel ledger.

**GC-5 — Human Explorer UI (PRIORYTET 7, graphics).** `components/HumanExplorerPanel.tsx`: a DOM dock over the existing biology `<canvas>` (no second renderer) with a body-systems rail (the atlas's 12 `OrganSystemId`s), an organ card (atlas node data — scale, dimensions, epistemic label), a magnification ladder (`VIRTUAL_MICROSCOPE_MAGNIFICATIONS`, 1×–1000×) and a macro→micro strip (`SCALE_LADDER`, now 8 rungs: BODY → ORGAN → TISSUE → CELL → ORGANELLE → MOLECULE → DNA → ATOM, extended in `core/scientificWorlds/humanExplorer.ts` with DNA running `central-dogma` and ATOM running nothing — no instrument in the lab models that scale, and the rung says so rather than fabricating a picture). Every click becomes canonical `WorldCommand`s on the same bus the command bar uses (`explorerCommands`, new `magnificationCommands`, `systemCommands`); the microscope field is a `<canvas>` drawn with the SAME `drawBiologyArtifact` the station screens use, fed only from the last sealed session that produced an image artifact — with no session it shows its named empty state, never a stock picture. Reused directly: the V3 anatomy atlas, visual modes, and the biology runners; `biologyStationKit`'s draw functions were generalised from a Three.js `CanvasTexture` readout to a small `ReadoutTarget` interface (context + canvas + a dirty flag) so the same drawer serves a scene texture and a DOM canvas. The reference image's organ photography/3D card is NOT built — no organ GLBs, histology datasets or microscopy imagery exist in the tree (**REQUIRED FROM USER** if that art direction is wanted); today the panel is data-driven from the atlas and sealed sessions only.

**GC-6 — Flagship agentic runtime (the "Agentic Science World OS" pack).** New `packages/core/src/flagship/` (canonical SHA-256 throughout, via the same `sha256hex`/`stableStringify` the EvidenceLedger uses — not the pack's reference FNV hash):
- `sessionEventLog.ts` — an append-only, hash-chained per-session trace (`SessionEventLog`, `replaySessionEvents`, `verifySessionChain`, `SessionEventLog.fromEvents` which refuses a broken chain); a trace of what happened, not a second Evidence Ledger.
- `epistemicGuard.ts` — `guardWorldMode`: a world mode's ceiling (`SCIENTIFIC` → REAL_OBSERVATION … `FICTIONAL` → FICTIONAL) that only ever lowers a requested status, never raises one — the epistemic firewall the pack's E2E-12 asks for.
- `mirrorTwin.ts` — the Mirror Twin as a pure state machine (`MIRROR_IDLE → SCANNING → SYNCING → TWIN_READY → DIVERGENCE_MODE → CAPTURE → REPLAY`) on the EXISTING `GenesisMirrorBridge` contract (consent required, no raw image, ephemeral payload, `VISUAL_SESSION_PROXY` — never an identity claim); an illegal event is refused into a `refusals` list, never thrown into the scene; divergence (the twin performing a different action than the subject) is the one required feature.
- `portal.ts` — an original Circular Gate / Phase Tunnel mechanic (concept-only; no franchise assets) as an 8-phase sequence a caller cannot skip; the destination's epistemic status is capped by `guardWorldMode`; the tunnel's decorative warp shading reproduces the Schwarzschild-radius/warp-factor formulas locally (not imported from `GenesisSpacetimePortalEngine.ts`, whose top-level `node:crypto` import cannot be bundled for the browser — importing it broke the Vite build; the physics is identical, just duplicated to avoid that coupling).
- `timeMachine.ts` — three modes sharing one function and never mixing truth status: `SCIENTIFIC_MODEL` computes ONLY a supported clock comparison (special-relativistic + weak-field Schwarzschild rate, SI `c`) and refuses anything else by name; `SPECULATIVE_PHYSICS` requires written assumptions or is refused; `FICTIONAL_UNIVERSE` makes no claim and carries the existing fiction disclaimer (reproduced locally for the same `node:crypto`-bundling reason as above).
- `cosmosObserver.ts` — an authorised observation enters the SAME EvidenceLedger as an `observation` claim with source/version/hash; a newer version is a NEW record that names what it supersedes (`ingestCosmosObservation`) — nothing is ever overwritten; a derived interpretation is a separate `model` claim (`recordCosmosInterpretation`). Nothing here fetches.
- `capturePreset.ts` — one shared capture spec (9:16/16:9/1:1 frames with safe insets, captions, camera move, and a mandatory epistemic badge derived from the status) for every flagship sequence; no capture without a replay key.
- `spacetimePhoton.ts` — the flagship physics scenario: first-order Shapiro delay and light deflection in a weak field vs. a flat (M=0) baseline, checked against the textbook solar-limb values (deflection ≈1.75″); `c = 299 792 458 m/s` is read as the SI-defined constant and never computed — the model compares two models, it measures nothing. Wired as kernel provider `spacetime-photon-model` and as the lab's observation-window experiment (`experimentRunners.ts`, station `st-window`).
- `core/scientificWorlds/agenticScienceRuntime.ts` (frontend) — `runAgenticScience`: perceive (the scene's own entities, labelled MODEL) → `bridge.attach()` into the cognitive core's WorldModel → reason (a hypothesis pair on the scenario's decisive observable, `shapiroDelayS`) → plan (`planActions` on typed `WorldCommand`s) → act (`AgentController` stepped headless — walk, align, reach, execute, observe, report) → ONE `ExperimentSession` at the window station → falsify (each hypothesis against the sealed output; `beliefRevision.updateConfidence`) → evidence (the session's ledger-committed hashes) → memory (`core.ingestObservation`) → answer (`buildTruthResponse` over the ledger, D-129) → every step on the hash-chained `SessionEventLog`. An action with no named human approver is refused (`ACTION_REQUIRES_HUMAN_REVIEW`) before anything runs. `runFlagshipJourney` composes the whole pack's journey — session start, world creation, mirror sync + divergence, gate traversal, a scientific-mode time machine, a cosmos observation, the agentic loop, a capture spec, then replays the event log and asserts it equals the live state, and replays the sealed session (MATCH). Wired into `ScientificWorldsScreen` (physics world) as a one-click "Pętla agentowa: foton (zatwierdzam)" button — the click is the human approval.

**Not integrated from the packs (stated, with reasons).** `taxVatIntelligence.ts` (a separate regulatory vertical outside the science stack; no owner decision, per D-129's BLOCKED list) and the `fundamentalPhysicsResearch.ts` catalog of physics tracks and "verified" external data sources (CERN Open Data, NIST, etc.) — publishing a "verified sources" list without actually checking each URL/terms would itself be an unverified claim; the one scenario the pack asked to be flagship (weak-field photon propagation) is implemented and wired, the rest of the catalog is left as pack documentation, not code, until a real connector is built and each source is checked.

**Tests.** `packages/core/src/knowledge/ledgerPersistence.test.ts` (snapshot→restore reproduces the chain bit for bit, a tampered snapshot is rejected and left on disk, an empty store starts EMPTY), `packages/backend/src/knowledgeLedgerPersistence.test.mjs` (proposals and published records survive a real process restart via `openKnowledgeLedgerPersistence`, a tampered file is rejected), `packages/frontend/src/__tests__/kernelLedgerPersistence.test.ts` (localStorage round-trip), `curiosityCycle.test.ts` (a cross-host contradiction resolved within the model by the epidemiology console end to end — blocked without approval, nothing runs; approved — hypotheses revised, evidence on the ledger, replay MATCH, Science Memory written, the next run does not repeat the question; honest endings for no-key and no-instrument questions), `packages/core/src/knowledge/molecularBiology2.test.ts` (all seven mechanisms + the network + the extended provider), `environmentalCaseGraph.test.ts` (candidate vs. every INSUFFICIENT_EVIDENCE path; dangling edges, duplicate ids and out-of-range uncertainty refused), `humanExplorer.test.ts` (extended: DNA/ATOM rungs, magnification/system commands, `levelOfSession`), `packages/core/src/flagship/flagship.test.ts` (the event log's determinism/tamper-detection/replay-equality, the epistemic firewall never raising a status, the mirror's consent gate and divergence, the portal's phase sequence, all three time-machine modes, cosmos versioning with no overwrite, the capture spec, and the spacetime-photon model against the textbook solar values), `agenticScience.test.ts` (the full loop on the real lab services headless — perceive through replay; refuses to act without approval; a fictional world caps the answer; the full flagship journey with replay equality and session MATCH; two independent runs of the same journey hash identically).

**Verification.** Frontend, backend and core+ui+e2e suites pass; lint 0; tsc clean; production build OK (fixed one accidental `node:crypto` coupling the flagship layer introduced, caught by the build, not by tests); the Human Biology Lab E2E re-run end to end with the Human Explorer panel open (a click through BODY→CELL, a new Hyperscope session sealed, six screenshots including the new explorer panel, zero console errors).

## D-131 — Human Digital Twin: the licensed CC0 body, a real section plane, isolation and the fresnel effect layer (no new asset bought, no scientific change)

**Audit of the delivered pack** (`GENESIS_Human_Digital_Twin_Research_Edition_CLAUDE`, 292 lines of TypeScript). It contains no model, no anatomy, no textures and no microscopy data, and says so itself: "Pakiet nie zawiera licencjonowanego modelu medycznego. To świadomy asset gate." Five of its seven modules duplicate systems this repository already has (Human Explorer → D-129/D-130, Mirror Twin → D-130, anatomy/cutaway controllers → empty ports over a renderer that did not yet implement them, contracts → `HumanDigitalTwinManifest`). Two ideas were worth taking: **the asset gate** (a licensed model may load only once provenance, licence, checksum and permitted uses are on record) and its **acquisition checklist** (a negotiation document, not code). The pack also names two files it does not contain — `RealDataProvenancePipeline` and `AssetLicenseRegistry` — which exist in no pack and in no branch; nothing was invented to fill them.

**The finding that made this iteration possible.** D-127 recorded "the repository's only human model (`mpfb-lod0.glb`) is UNVERIFIED in `assetGovernance`", and the governance record's own rationale read "brak lokalnego rekordu źródła i licencji". That rationale was wrong: the record existed, in `public/assets/genesis-hf/ASSETS.md`, beside the file. Two facts were verified before anything was changed: (1) both SHA-256 sums recomputed from the files in the repository match `ASSETS.md` exactly (`63c645a2…` / `ec47cffd…`); (2) the upstream README was fetched and quotes verbatim: *"Example avatar »mpfb.glb« was created using Blender and MPFB Blender extension. The avatar is licensed under CC0."* (MPFB is described there as using the MakeHuman ecosystem, CC0/CC-BY). Only then was the record promoted to `APPROVED` with its real source, licence, licence URL, author and both checksums. Inspecting the GLB shows what is actually in it: 8 meshes, a 67-joint skin, 66 ARKit-style facial blendshapes (visemes, `eyeBlinkLeft/Right`, brows), 7 WebP textures. **It is an OUTER HUMAN — skin, clothing, hair, teeth, an animation skeleton. It contains no organs, no bones as geometry, no vessels and no nerves, and is never presented as medical anatomy.**

**What changed.**
- `core/three/humanTwinAsset.ts` (NEW) — the pack's asset gate, implemented over the ONE canonical registry (`assetGovernance`), adding no second registry: it approves a load only for an `APPROVED` record that carries a licence, a licence URL, a source URL and a checksum **for the very file being fetched**, and returns the tier (`LICENSED_CC0_ASSET` / `PROXY`) plus a refusal reason otherwise. The gate runs BEFORE the fetch, so an unapproved asset causes no network request at all. `humanTwinProvenanceLabel` keeps the two facts apart in the HUD: what the body is made of, and what the anatomy is worth (always MODEL).
- `core/three/biologyLabKit.ts` — `createTwinProxy` takes an optional `bodyAsset`. With the approved asset the GLB **is** the body (the procedural rig is still built, because the `Character` handle is part of the contract, but hidden — there is never a second body on screen); without it, the previous proxy is unchanged. The handle gained `tier`, `setIsolated`, `setCutaway` and `setSurface`.
- `core/three/humanTwinCutaway.ts` (NEW) — the section plane that did not exist. `cutawayEnabled` had been live state in the V3 anatomy reducers since D-127, and `clippingPlane` a field on every visual-mode instruction, but **no clipping plane existed anywhere in the renderer**: the flag was dead. Now three anatomical axes (sagittal / coronal / axial), a position that travels inside the twin's own measured bounds so a cut always meets the body, a flip, a faint plane indicator, and `renderer.localClippingEnabled` driven on only while a cut is active (the opaque path is untouched otherwise).
- `core/three/humanTwinMaterials.ts` (NEW) — one fresnel term, injected into the asset's own PBR materials through `onBeforeCompile` (the licensed material is never replaced), driving both the rim light that separates the silhouette from a dark lab and the X-ray shell that fades head-on so organ proxies read through it; plus the selection pulse and the shared easing.
- `agentLabScene3D` — loads the approved asset asynchronously after `initBiology` and swaps the proxy for it; a refusal, a missing file or a decode error simply leaves the proxy standing. New scene API: `getTwinTier`, `setTwinTierListener`, `setTwinIsolated`, `setTwinCutaway`.
- `ScientificWorldsScreen` / `HumanExplorerPanel` — the existing Human Explorer gained section controls (cut, axis, position slider, flip) and isolate/show-all; **no second Human Explorer was created**. `applyAnatomyInteraction` routes two more actions through the existing V3 reducers (`SET_CUTAWAY`, `ISOLATE_NODE`/`CLEAR_ISOLATION`), so the same WorldCommand bus drives them; an unknown node is still refused with a reason rather than invented.
- The asset's own `eyeBlinkLeft/Right` blendshapes give the twin a blink, deterministic in scene time. It is presentation only: it never enters a session, an experiment input or an evidence record.
- **A third camera, `TWIN`** — and the reason it had to exist. The first browser run rendered the licensed body correctly and photographed *nothing*: both existing cameras (VISOR, SPECTATOR) follow the suited agent, which leaves the twin a centimetre-high figure behind the glass of its chamber. A section plane and an isolated organ are unreadable at that distance, so every claim about them would have rested on code review alone. `AgentCameraMode` gained `TWIN`: a still shot framing the chamber, moving in when the view narrows (an isolated organ, an active cut) and easing back out when it widens, with the twin turning on its own because it already is one of the scene's slow spinners. It is a camera and nothing more: no label, session, epistemic status or evidence record changes with it. The Human Explorer carries the toggle (`Kamera bliźniaka`), and the HUD names it.
- **The X-ray / surface layer, finished rather than merely written.** `setSurface` existed on the twin handle from the first pass of this entry and *nothing called it* — the fresnel X-ray shell was dead code, exactly the kind of thing that reads as delivered in a diff and does not exist in the product. The scene now owns the surface mode (`setTwinSurface`, preserved across the asynchronous asset swap) and the Human Explorer exposes all four: `Skóra`, `Prześwit`, `RTG (model)`, `Duch`. The X-ray chip names itself a model so the control can never be read as a radiograph, and the E2E asserts that switching to it leaves `ANATOMIA: MODEL` standing.
- **Content-Security-Policy: `blob:` in `img-src` and `connect-src`.** `GLTFLoader` unpacks the textures embedded in an already same-origin `.glb` into `Blob`s and reads them back through `URL.createObjectURL` (`ImageBitmapLoader` does that read with `fetch`, hence `connect-src` and not only `img-src`). Under the previous policy the browser refused all seven textures and the body rendered untextured, with seven console errors — invisible to every unit test and caught only by the browser E2E, which asserts a clean console. A blob URL is minted by the page itself from bytes of its own origin and cannot be pointed at anyone else's server, so this is not an exfiltration channel; `lib.test.mjs` now pins both the allowance and its limits (no `http:`, no `https:`, no `*`, no `unsafe-inline`, no `unsafe-eval`).
- **HUD layout.** With the explorer dock open, the bottom-left transcript overlapped the dock's left column and intercepted its clicks — a real defect, found by the E2E rather than by any test of the components. The transcript now takes exactly the width left over beside the dock and the 12.5 rem nav rail, so the two cannot overlap at any desktop width.

**What is still NOT anatomy, stated plainly.** The organ shapes remain atlas ellipsoids (MODEL). There are still no vessels, no nerves, no muscles and no bone geometry, and none were faked to fill the gap — the VASCULAR and NERVOUS display modes tint the organs they always tinted. A cut reveals those model volumes, so the section is a SCHEMATIC section of a model, never a medical cross-section of a patient; the panel says exactly that under the slider. The X-ray view is a fresnel shell, not a radiograph. Histology, cells and microscopy are unchanged: `preparationStatus: 'VIRTUAL'`, `epistemic: 'MODEL'`, seeded and procedural. `REAL_IMAGE` and `REAL_DATASET` remain values that nothing in the repository produces. A better-looking body upgraded no epistemic status anywhere.

**Verification.** Lint 0; `tsc --noEmit` clean; production build OK; frontend 599 files / 6776 tests pass; core 46 files / 465 tests pass; backend 862 tests, 0 failures. The Human Biology Lab E2E was run end to end in a real browser four times across this iteration and passes, capturing nine screenshots including the three new ones (`human-biology-lab-twin-cc0.png`, `human-biology-lab-xray.png`, `human-biology-lab-cutaway.png`), with zero console errors. Each screenshot was reviewed by eye before any commit, and that review is what produced the last four fixes in this entry: the browser proved the licensed asset loads and the section cuts, and the pictures proved the app was not yet SHOWING either.

**Tests.** `humanTwinAsset.test.ts` (the gate approves only a complete verified record and refuses a missing record, an unapproved record and a missing checksum by name; the HUD label keeps body and anatomy apart and can never read as a medical claim; the section plane's axes, travel inside bounds, flip symmetry and clamping; the effect helpers' determinism; the two new anatomy actions through the V3 reducers, including the refusal of an unknown node). `assetGovernance.test.ts` — the old test asserted the human "stays blocked" *because it was undocumented*; that premise is now false, so it asserts the stronger invariant instead: approved ONLY with licence, source, author and the file's own checksum, `approvedAssetsMissingProvenance()` empty, the rationale explicitly stating it is not medical anatomy, and an asset with no recorded source still blocked. The biology E2E additionally asserts, in the running browser, that the twin reports `LICENSED_CC0_ASSET`, that the HUD still says `ANATOMIA: MODEL`, that the section toggles for real, and that cutting and isolating seal no session.

## D-133 — Smart UI, phase 1 (Molecule World): WORLD VIEW → hover → Contextual Popup → Research Drawer, reusing the existing InteractionController (no scene change, no second design system)

**Audit before integration.** A delivered "Cinematic Reality Engine" / Smart UI pack (`GENESIS_SMART_UI_CODE_FOR_CLAUDE.zip`) was reviewed line-by-line against the live repo before any file was touched. It introduced a workable World View → Contextual Popup → Research Drawer interaction shell (`WorldViewShell`, `ContextualPopup`, `ResearchDrawer`, `EpistemicBadge`) but also: a fourth, competing epistemic-tone vocabulary alongside `.gx-status`, `ExplorerEvidenceMode` and `EvidenceLedger.ClaimType`; a popup that centred blindly on its anchor and could be clipped off-screen near the top of the viewport; a `role="dialog"` with no focus management; two separate `.gx-world-first-shell{}` CSS blocks with hardcoded colour fallbacks instead of this repo's real `--text`/`--bg` tokens; and React Testing Library, a testing methodology this repository has never used in ~600 existing test files (`renderToStaticMarkup` + string assertions is the established convention). None of these were integrated as delivered; each was fixed as part of landing the pack (below).

**What this phase is, precisely.** Molecule World (`MoleculeLabScreen.tsx` / `moleculeScene3D.ts`) was chosen as the proof-of-pattern world because it already had a real `InteractionController`/`WorldFrameRenderer` raycast pipeline (`onSelect`) from an earlier pass — this phase adds `onHoverChange` beside it and swaps the screen's permanent, always-visible `WorldChrome` header + fixed-corner detail card for the World-First contract: the 3D stage fills the screen by default; hovering or clicking a real RDKit atom raises a small Contextual Popup anchored to its live screen position; its **BADAJ** (research) action opens a Research Drawer (desktop: side panel: `min(30vw, 480px)`; mobile, `<900px`: a bottom sheet, `max-height: 78vh`); closing restores a clean World View. Nothing about the RDKit materialisation, the WorldGraph/TemporalEngine orchestration, the camera, or Phase 8.1's real bond rendering changed — this is presentation and interaction only, exactly the directive's "never change the 3D scene just to mount a Smart UI."

**What changed.**
- `components/genesis-ui/` (NEW) — `worldFirst.types.ts`, `EpistemicBadge.tsx`, `ResearchDrawer.tsx`, `WorldViewShell.tsx` kept as delivered (audited, no defect found); `ContextualPopup.tsx` rewritten to clamp its position fully inside the viewport (measuring its own rendered size via `useLayoutEffect`, flipping below the anchor when there is no room above) instead of a fixed `transform: translate(-50%, calc(-100% - 12px))`, and to use `role="group"` with a real close-button label instead of an unmanaged `role="dialog"`; `worldViewShell.css` merged into one rule block per selector, aliased to this repo's real `--text`/`--bg` tokens, dead transform rule removed. `epistemicToneAdapter.ts` (NEW, original) is the ONE translation point from `.gx-status` tones and `ExplorerEvidenceMode` into the shell's presentation tone set — no fifth vocabulary invented, each source stays authoritative for its own layer.
- `core/three/moleculeScene3D.ts` — additive only: `onHoverChange` next to the existing `onSelect` in the `InteractionController` constructor (`handleHover`, reusing `applyHighlight`/`clearHighlight`, already used for selection); `screenAnchorFor` projects the selected atom's live world position to screen pixels (`getWorldPosition().project(camera)`, `null` when behind the camera), read into `getStats()` as `selectedAnchorX/Y` — the SAME throttled (~250 ms) `onStats` channel `useThreeLoop.ts` already drives, not a second per-frame update path. New `suspendAutoRotate()` (see below).
- `core/three/types.ts` / `useThreeLoop.ts` — one new optional `Sim3D.suspendAutoRotate()` hook. **The bug it fixes was found by the E2E, not by any unit test**: the molecule's existing cinematic idle auto-rotate (`cameraAutoRotateSpeed = 6`) kept sweeping the selected atom's screen position every frame, so the Contextual Popup anchored to it — and its BADAJ button — never stopped moving; a real Playwright click on BADAJ retried against a moving target for the full test timeout without ever landing. `suspendAutoRotate()` lets a `Sim3D` suppress the idle auto-rotate while something is selected, combined each frame with the existing drag/idle state machine so neither overrides the other; the default (hook absent) leaves every other `cameraAutoRotateSpeed` scene unchanged. `MoleculeScene3D.suspendAutoRotate()` returns true exactly while an atom is selected.
- `MoleculeLabScreen.tsx` — rewritten onto `WorldViewShell`; the old unconditional `WorldChrome` header and fixed-corner `gx-floating-card` are gone. `onObjectAction` needs no explicit drawer-opening logic for the research action — `WorldViewShell`'s own `onContextAction` already opens the drawer for any action of `kind: 'research'` (caught and removed during implementation after first duplicating it). A `frameTick` counter, incremented on the same throttled `onStats` tick every other `Sim3D` screen already uses for its own `data-frames` counter, backs the E2E's frame-settledness wait (`data-testid="molecule-world"`) without adding a second render-loop tap.
- `main.tsx` — one new stylesheet import (`worldViewShell.css`).

**Real bugs the browser found that no unit test could.** (1) `ContextualPopup`'s `useClampedPosition()` measured its OWN internal `ref`, but the rendered `<section>` was wired to a different, unused `ref` declared in the outer component — `ref.current` inside the measurement effect was therefore always `null`, so the popup never left `visibility: hidden`. `renderToStaticMarkup` tests cannot catch this (there is no DOM, no ref, no layout); only a real browser click proved the popup silently never became visible. (2) the auto-rotate/anchor race above. Both were fixed and re-proven by a second real run before this entry was written — per this repo's own rule, neither is claimed fixed on code inspection alone.

**Tests.** `worldViewShell.test.tsx` (NEW, `renderToStaticMarkup`, this repo's own convention — zero React Testing Library anywhere in ~600 existing test files): default World View carries no popup/no drawer; a controlled selection renders the popup with the object's label, subtitle and BADAJ action, never the drawer; the epistemic badge carries its tone as a data attribute, never upgraded; the tone adapters never invent a value. `moleculeScene3DPicking.test.ts` extended with hover tests (a real atom mesh fires `onAtomHovered` without selecting it; moving off fires `onAtomHovered(null)`; hover and selection stay independent) and screen-anchor tests (`NaN` with nothing selected, a finite anchor matching the atom's real projected screen point once selected, back to `NaN` on deselection). `packages/e2e/src/molecule-world.e2e.spec.ts` (NEW) — a real browser, against the production server, proving what a static render cannot: `/#/molecule` reaches a real materialised RDKit molecule (waited for explicitly — rendered frames alone do not prove materialisation finished, a real network round trip to the RDKit backend); a dense grid-scan finds a real atom mesh under the pointer (no hardcoded atom position); hovering fires the hint; clicking opens the popup fully inside the viewport with an honest `REAL (RDKit)` / `NOT_MODELLED` badge; BADAJ opens the drawer and the popup yields to it (never both at once); Expand/Collapse toggle the drawer's expanded class; Close restores `data-state="world"` with no popup and no drawer; the scene's own honest RDKit status line survives the whole sequence unchanged. A second test proves the mobile (`390×844`) drawer becomes a bottom sheet pinned to the bottom of its own container (below `TopBar`, above the mobile tab bar — not the raw viewport edge), not the desktop side rail shrunk to fit.

**Verification.** Lint 0; `tsc --noEmit` clean; frontend 601 files / 6797 tests pass, 1 skipped; production build OK. `packages/e2e/src/molecule-world.e2e.spec.ts` passes both tests (desktop and mobile) against the real production server in a real browser, zero console errors in either run. The `useThreeLoop.ts` change is scoped and behaviour-preserving for every scene that does not implement `suspendAutoRotate()` (currently only `MoleculeScene3D`) — verified by the full frontend suite passing unchanged and by code inspection of the one new conditional; the broader existing 3D-world E2E suites (CERN, Scientific Worlds) were not re-run in full for this entry, since `CernComplexView` does not use `useThreeLoop.ts` at all and the change to scenes that do is a no-op absent the new opt-in hook.

**What is still NOT done.** Phases 2–9 of the Master World Visual Completion directive (Human Biology + Scientific Worlds Smart UI, Scientific City/Genesis World/Virtual Lab, CERN, Cell Lab's Canvas-2D→3D gap, the shared `WorldVisualRegistry`/`WorldSceneComposer`, the remaining nine world environments, the WorldProposal→WorldVisualDefinition connection, full visual regression) are not started. This entry closes only Phase 1 (Molecule World) with real, re-run evidence.

## D-134 — reconciling a numbering collision: two independent sessions each wrote a "D-109"; the genuinely new value from the second is carried forward, nothing else is duplicated

**Why this entry exists.** During branch consolidation, `claude/genesis-autonomous-completion-95bt4e` was found to carry its own decision-log entry titled "D-109 — full dependency-graph audit toward the first legitimate WinnerRecord" (dated 2026-09-16), written independently of and in parallel with the "D-109 — Winner Gate / ResearchRecipe bypass audit; the D-081a header comment goes stale, fixed" entry already on `main` (commit `49cc9084`). Two sessions picked the same free number without visibility into each other. Per the consolidation mandate: keep exactly one thing called "D-109" (`main`'s own, left untouched above, at its original place in history — decision-log entries are not rewritten after the fact), and give this entry, which reconciles and carries forward the second session's *new* content, the next actually-free number.

**What the two audits agreed on, independently — not duplicated here.** Both sessions, working from different starting points, reached the same conclusion on the Winner Gate: `canPromoteToWinnerRecord` (`winnerGate.ts`) is defined once, imported by every real caller, no hardcoded WINNER/PROMOTE literal exists outside tests, and `syntheticWinnerFixture.ts` never reaches the real Mounjaro path. `main`'s own D-109 entry already records this; it is not repeated here. Re-verified today (2026-09-21) by execution, not assumption: **`winnerGateBypassAudit.test.ts` — 11/11 pass, fresh run on current canonical code.**

**What main's D-109 did NOT cover, and is genuinely new here.**

1. **TRIAL_2_STATUS, precisely defined.** D-106/107/108 described the remaining D-088 attempt as "awaiting one authorization," which reads as if a single approval would run an already-defined experiment. Re-reading D-088's own sealed prereg text directly shows this is imprecise: D-088 already executed both arms once, deterministically, under a seal whose own `decisionRule.forbidden` clause bars re-running it with any different knob, and whose `armsAreExhaustive` clause states a further arm needs a **new** human-sealed preregistration, which does not exist. `scripts/d109-trial2-readiness.mjs::trial2Status()` reads this outcome directly from the sealed prereg's own text (not a restated copy) and reports `BLOCKED` with that reasoning. **Verified by execution today**: `d109Trial2Readiness.test.mjs` tests 1–3 (TRIAL_2_STATUS logic, attempt-budget interpretation, "proposes no new prereg") pass fresh against current code.
2. **The exact GIPR data-volume gap, with a minimum-payload estimate.** `giprActivity.json`'s 233 rows split (scaffold-hash, deterministic, unchanged since D-076) to nTrain=146 (need ≥150) / nTest=24 (need ≥40) — main's D-109 states the same counts descriptively in its comment fix; this entry adds the estimate that closing the binding (test) shortfall needs on the order of ~150-160 additional real rows, given the empirically observed ~10.3% test-bucket rate — stated as a scale estimate for the owner, not a target to game the split against.
3. **Dual-target discovery, audited against the real current probes** (not fixtures): `assessDualTargetAxes`/`dualTargetVerdict` (`dualTargetDiscovery.mjs` — pre-existing shared module, not new) correctly returns `MECHANISM_INCOMPLETE` given `GLP1R_ACTIVITY_UNAVAILABLE`, `GIPR_ACTIVITY_UNAVAILABLE` and the separately-blocked `PRIOR_ART_UNVERIFIABLE` egress check — no fabricated axis, no candidate manufactured. Not covered by main's D-109 at all.
4. **Eleven pinned regression tests for the bypass-audit conclusion** (`winnerGateBypassAudit.test.ts`) that did not exist on `main` before this entry — main's own D-109 audited the same properties but left no regression suite pinning them, so a future change reintroducing a bypass would previously have shipped silently.

**What was carried forward, file by file:** `scripts/d109-trial2-readiness.mjs`, `packages/backend/src/d109Trial2Readiness.test.mjs`, `packages/frontend/src/__tests__/winnerGateBypassAudit.test.ts`. The GIPR header-comment fix and the bypass-audit prose itself were **not** re-applied — `giprQsar.mjs` already carries the corrected numbers via `main`'s own D-109, and duplicating the same comment edit a second time would be exactly the kind of redundant parallel work this consolidation exists to remove.

**Verification, precisely separated by method (nothing here is inferred or assumed):**
- **Verified by execution, today (2026-09-21), on current canonical code:** `winnerGateBypassAudit.test.ts` 11/11 pass. `d109Trial2Readiness.test.mjs` tests 1–3 (TRIAL_2_STATUS/attempt-budget/no-self-authorization) pass. `tsc --noEmit` clean.
- **Verified by code/diff inspection, today:** `winnerGate.ts`'s single-definition/single-caller structure, `dualTargetDiscovery.mjs`'s pre-existing presence in canonical, and `giprQsar.mjs`'s header already carrying the corrected 233/219/146/24 numbers (confirmed by reading the file directly, not by trusting either decision entry's prose).

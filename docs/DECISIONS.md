...
**Not done, stated.** `LaypersonAssistant` and `sovereignTruthAnswer` remain unrouted (a product decision on which surface answers with them); the EvidenceLedger stays in-memory; no organism-specific biology (no splicing, no folding solver); the detective has no UI and no real dataset in the tree; hazard registry adapters remain NOT_IMPLEMENTED.

...
Where it lives: the chemistry panel of the ONE main Laboratory
(`#/scientific-worlds`). Open it with the Laboratory's "Chemistry" chip, or
from Tryb badawczy → Chemia → "Chemia — stanowisko miareczkowania"
(`#/scientific-worlds?station=st-titration`). There is no separate route. A
titration started in the panel also runs at the Laboratory's titration
station, through the same shared `runTitrationScenario`.

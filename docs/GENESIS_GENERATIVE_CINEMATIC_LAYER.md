...
The local-first Stage A backend is now connected through
`GET /api/compute/local-video/runtime` and `POST /api/compute/local-video/plan`.
World Director can run this admission check against the current canonical
world ID and scientific-state fingerprint. On the verified Windows host it
detects the AMD Radeon RX 7700 XT and bundled FFmpeg, but returns
`BLOCKED_MODEL_UNAVAILABLE` because no local model/checkpoint is registered.
There is intentionally no production generation endpoint yet.

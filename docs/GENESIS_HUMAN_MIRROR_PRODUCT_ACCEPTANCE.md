...
1. **Desktop, no camera permission**: `MIRROR_IDLE → CONSENT_REQUIRED`, consent click ⇒
   ...
   `STREAM_OPEN`.
2. **Desktop, full OS/browser permission + Chromium fake video device**: the scoped
   `camera=(self)` policy permits the real browser stream to open. The UI reaches `SYNCING`
   while remaining explicitly `SYNTHETIC_FALLBACK / NOT_CALIBRATED`; it never claims
   `MEDIAPIPE`, face tracking, physical calibration, or identity.
...
### Integrated camera policy and remaining validation boundary

The integrated branch scopes the response policy to `camera=(self)`, while microphone and
geolocation remain disabled. This closes the software-level browser capability blocker and
allows a consented same-origin `getUserMedia` stream.

The stream is still not physical-validation evidence. Real landmark extraction, camera
calibration, face/world registration, and validation on a physical camera remain unproven.
Until those steps are completed the product must continue to display
`EXPERIMENTAL / SYNTHETIC / NOT_CALIBRATED` and `LOCAL_CAMERA_VALIDATION_REQUIRED`.

...
| 2. `/` i `/matrix` = pełnoekranowy WebGL + bezramkowy HUD | Jeden silnik „czarna cyber-przestrzeń": proceduralny wolumetryczny deszcz glifów liczony w całości na GPU (kolumny w głębi, głowy i ogony, migotanie, zanik z odległością), czarne lustro Reflector bez tintu + hairline'owa siatka, czarna mgła (bez zielonej poświaty). `/`: sam kod nad czarnym tłem, HUD w 100 % przezroczysty (hairline'y, bez slabów, bez blur). `/matrix`: ta sama scena z niską kamerą nad lustrem (odbicia glifów w obsydianie, podwójny bloom tight+wide tylko na znakach, ACESFilmic), jedna kolumna HUD po prawej, zero kart. **Dyrektywa absolutna właściciela (18.09, po południu): ZERO postaci, manekinów, piedestałów i kolumn słów** — wszystkie usunięte z kodu WebGL (`holo/MatrixStage.ts` = wyłącznie deszcz + lustro + siatka). Lewe ciężkie menu → pływająca, bezramkowa, w 100 % przezroczysta szyna nawigacji (sekcja 12 `styles-2040-hud.css`). Dawna mapa systemu → `#/matrix-map`. Wcześniejszy portal FBO usunięty na żądanie właściciela (tło ma być czarne z cyframi). | zrzuty `genesis-matrix-stage.png`, `genesis-root-hud.png`; test `matrixStageView` (brak `.matrix-card`) |
...
**Addendum (18.09, noc) — Kompleks CERN 5D.** Trasa `#/cern-complex` (pełnoekranowy WebGL: hub laboratoryjny, pancerna szyba z refrakcją, segment pierścienia LHC; potok SSAO + SSR + bloom + rozpraszanie wolumetryczne + soczewkowanie/szyba; tryby kamer 1–4, akcje Q/E/R). Trzej nowi dostawcy jedynego kernela: `micro-blackhole-sim` (etykiety HYPOTHESIS / SPECULATIVE / NOT_FORMED od silnika), `crystal-synthesis-sim` (`EMPIRICAL_ESTIMATE_MODEL`), `collision-batch` (`TOY_MC_MODEL`); każdy wynik kotwiczony w EvidenceLedger kernela. „Oczy": `packages/e2e/src/cern-complex.e2e.spec.ts` + `VisualFidelityHarness` (dekoder PNG, progi: wariancja luminancji, czerń, dominacja zieleni, gęstość krawędzi, głębia kolorów) — cztery zrzuty `artifacts/{walk-5d,glass-refraction,console-pbr,tunnel-volumetric}.png`; pierwszy render był prześwietlony i został poprawiony ręcznie w trzech przebiegach (ekspozycja, natężenia świateł, pozy kamer). Szczegóły i lista poprawek dostarczonego kodu: `docs/DECISIONS.md` D-126.

...
This document defines the **read-only contract** used by the product renderer.
The scenario module itself remains free of rendering, Three.js and UI code.
Production binding now lives in
`core/worldDirector/sw4WorldDirectorAdapter.ts` and the existing
`WorldDirectorScreen` / `TemporalCinematicSim3D` path.
...
### Product binding (now wired)

An explicit SW-4, SEIR or epidemic-city prompt in the existing World Director
is resolved by `directSw4World`. That adapter independently rebuilds the run,
requires replay verdict `MATCH`, derives a camera from the same opt-in
structural city geometry and hands the exact advanced `TemporalEngine` to
`TemporalCinematicSim3D`. The existing `WorldFrameRenderer` therefore renders
the same `WorldGraph` whose population was advanced by `epidemic-seir-rk4`.
The UI displays S/E/I/R, tick, simulated day, solver id, replay verdict and the
full model disclosure; it does not maintain a second epidemic state.
...
- The scenario module does not decide which screen invokes it; the product
  binding is isolated in `core/worldDirector/sw4WorldDirectorAdapter.ts`.

...
The latest published code commit is `9534d8d` on `manus/high-fidelity-epidemic-digital-twin`. The latest CI for that commit is `33086705393` and is green. The accepted Claude handoff is `a23e904` (Epidemic→City3D same-world handoff plus routing proofs), the Evidence/Replay entry point is `460c5cf`, the ontology audit and package contract are `c6f612b`, the visual language is `430a0b1`, the preregistered Protocol / A-B designer is `d4c6acd`, the WHY / Next Experiment layer is `5154b71`, the Campaign read-only adapter is `838ef42`, the Campaign reference fixture is `ce9efc6`, the water-pump-pipe Science Chat expose is `6952df6`, the PySCF H₂ RHF chemistry expose is `946f076`, the quantum-tunneling Science Chat expose is `b390e16`, the Earthquake Pilot honesty fix is `be529d3`, the CI/GIS boundary fixes are `7ad97ef`, and the Experiment Pilot Series Observation adapter is `a172487` (CI `33084839359`), backend provenance transparency is `461ee77` (CI `33085822920`), and Science Chat → Protocol/A-B navigation is `9534d8d` (CI `33086705393`).

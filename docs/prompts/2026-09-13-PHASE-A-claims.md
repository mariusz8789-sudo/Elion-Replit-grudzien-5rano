...
| **PHASE G — G2 DifferentiatingExperimentGenerator (mandat "Qwen" Phase G, podział 20/80 z użytkownikiem)** | **ZROBIONE (G2 only — 20% C1; G1,G3-G8 czekają na dopracowanie przez Qwen; G0 merge/deploy świadomie nieruszone, decyzja użytkownika)** | **C1, ta sesja** | patrz D-037 — `core/agent/differentiatingExperimentGenerator.ts`; **REUSE vs własne (jawnie):** próg `TAU_DISCRIMINABILITY` oraz `classifyObservationGap`/`createObservationGapRequest`/`undeclaredFeasibility` — reużyte bezpośrednio z M1 `observationGap.ts`; sama statystyka discriminability liczona w G2, bo M1 jej nigdy nie liczyło (`classifyObservationGap` tylko KONSUMUJE `bestDiscriminability` od wywołującego, dla 2 modeli) — G2 uogólnia tę samą miarę |Δ|/σ z 2 hipotez na macierz par N×N, bez drugiego progu i bez drugiej statystyki; wybór eksperymentu przez `falsificationPower` (frakcja par hipotez rozdzielonych), nie przez najgorszy przypadek — dwa błędy logiczne znalezione i naprawione faktycznym uruchomieniem testów; fixture DD-EXP z mandatu przechodzi w całości (X wybrane, H1 rozdzielone od H2/H3, H2-vs-H3 uczciwie nierozstrzygnięte, realny `followUpGapRequest` na Y w tym samym wywołaniu, reguła decyzyjna zamrożona przed obserwacją); **9/9 testów**; pełna bramka frontend 5821/5822 (1 skipped), backend 396/396, repro-demo 69/69 |
| **GOV-DRUG-DISCOVERY-CAMPAIGN-01 — pełne rządowe E2E na ekranie (TOP10→TOP2, ożywiona bramka bezpieczeństwa, wyczerpanie ścieżek przed brakiem zwycięzcy)** | **ZROBIONE** | **C1, ta sesja** | patrz D-038 — `core/agent/trialRegistry.ts` (G6.1, 15/15), `core/biotechData/govDrugDiscoveryCampaign{,Preregistration}.ts` (27/27), `components/GovDrugCampaignScreen.tsx` + trasa `#/gov-campaign`. **Naprawiony realny BŁĄD:** `practicalCandidateGate.ts` był martwym kodem na ścieżce lekowej, a jedyne istniejące wywołanie w A2 przekazywało `unresolvedContradictions: []` jako literał — kryterium `NO_UNRESOLVED_CRITICAL_CONTRADICTION` nie mogło odpalić NIGDY; teraz zasilane realną kontrewidencją z falsyfikacji i **odmawia obu finalistom**, przy teście negatywnym dowodzącym, że kandydat bez sprzeczności przechodzi. **NO_WINNER nie jest już jednoprzebiegowym stopem:** 4 zadeklarowane ścieżki wyczerpania, ścieżka `NOT_ATTEMPTED` → throw; selekcja dwuprzebiegowa z jawnym `exhaustionChangedVerdict`. **E2E-01 NIETKNIĘTE** (nadal 18/18, nadal `NO_WINNER`, odcisk `f528c881`) — nowy lejek dostał WŁASNĄ pieczęć dziedziczącą `E2E01_WINNER_RULES` dosłownie, bo `top3Size` siedzi w zapieczętowanym odcisku, a dane są już znane (edycja = HARKing). **Werdykt kampanii = `CONFLICTING_EVIDENCE` vs `NO_WINNER` w E2E-01 — zmienił się przez SZERSZĄ kontrolę (shortlist 8 zamiast TOP3, falsyfikacja 8×6=48 ataków), nie przez obniżony próg.** 2671 kandydatów → 20 → 8 → 8 → 2; α 0.05/48 = 1.042e-3 z rejestru prób (2744 zapisane próby); `npm run e2e:gov-campaign` **16/16**; **`npm run e2e:gov-campaign:browser` 12/12 — liczby odczytane z DOM Chromium porównane z niezależnym uruchomieniem silnika w Node**; pełna bramka frontend 5863/5864 (1 skipped), backend 396/396, repro-demo 69/69 |
| **PHASE G — SCIENTIFIC PROOF LADDER P0-P10 + Tier A/B/C + Certificate v2** | **ZROBIONE (offline)** | **C1, ta sesja** | patrz D-039 — `core/agent/{proofLadder,predictionRegistry,blindDataset,causalLadder,discoveryCertificate}.ts` (23+11+9+10+11 = **64 testy**). **REUSE/EXTEND/NEW:** proofLadder = NEW addytywny nad nietkniętym `discoveryContracts.ts`; predictionRegistry = NEW; blindDataset = NEW (realny throw na warstwie dostępu, nie konwencja); causalLadder = EXTEND nad istniejącym `causalInference.ts` (DiD/ITS/synthetic control już były — brakowało tylko słownika); certificate = NEW kompozycja, append-only. **Drabina dwugałęziowa** (reprodukcja P0→P1 kończy się tam; nowość P0→P3→P4→(P2+P5 razem)→P6→P7-P10) — poprawka realnego błędu projektowego w spec, złapana uruchomieniem testów. **Gałąź reprodukcji NIE MOŻE awansować do nowości** — strukturalnie, dowiedzione testem. **Luka AT31 zamknięta na warstwie prezentacji:** `assertTieredStatusText` wpięty w `printCertificate`, 5 istniejących emisji gołego `DISCOVERY` świadomie nietkniętych (wpieczone w zweryfikowane odciski). **Złapany latentny bug Fazy F:** zegar (`new Date()` w `literatureNoveltyAdapter`) przeciekał do `outcomeFingerprint` — test determinizmu przechodził dotąd przez szczęście; naprawione + test regresyjny z `vi.useFakeTimers` (zweryfikowano, że pada bez poprawki). Runtime: `npm run proof-ladder:demo` **10/10**, Kepler = `REPRODUCTION (Tier A_COMPUTATIONAL, max P1)`, QE4 = `UNKNOWN (Tier A_COMPUTATIONAL, max P0)` — drabina nie cofa lekcji QE4. Pełna bramka: frontend **5928/5929**, backend 396/396, repro-demo 69/69, e2e:gov-drug 18/18, e2e:gov-campaign 16/16 |
| **Audyt KLASY „zegar w odcisku" + strażnik behawioralny** | **ZROBIONE** | **C1, ta sesja** | patrz D-040 — 138 plików importuje `events/hash`, **13 dotyka zegara/losowości, każdy zbadany pod kątem PRZEPŁYWU do hasha**: **12 czystych, 1 przeciek (ten z D-039, już naprawiony)**. Cztery czyste moduły niosą jawne komentarze o tej regule — `falsifiedModelRegistry.ts` dokumentuje **ten sam bug naprawiony tam wcześniej** (`e070289f` vs `dfbba267`), co dowodzi, że to klasa, nie incydent. **Reguła zapisana wprost:** timestamp to proweniencja, może bramkować kolejność (`assertFreezePrecedesDataset` słusznie czyta `frozenAt`), **nigdy nie jest inputem hasha**. Strażnik `fingerprintClockIndependence.test.ts` (**7/7**) jest **behawioralny, nie lexykalny** — uruchamia 7 punktów wejścia dwa razy z zegarem przesuniętym o pół roku; uzasadnienie w pliku: skaner lexykalny buga z D-039 **by nie złapał** (zegar i hash w różnych plikach, wartość przez dwie warstwy). **Historia bez dryfu:** `399221f5`, `f528c881`, `f4804820`, `44f245c9` — wszystkie stabilne po naprawie |

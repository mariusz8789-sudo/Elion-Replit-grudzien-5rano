...
async function openFromMenu(page: Page): Promise<void> {
  ...
  await page.getByRole('button', { name: /Chemistry Live Lab/ }).first().click();
  await expect(page.getByTestId('chem-live-lab')).toBeVisible({ timeout: 30_000 });
  expect(page.url()).toContain('#/chemistry-live-lab');
}
...
test.describe('Chemistry Live Lab', () => {
  ...
  test('governed boundaries: unsupported reaction and hazardous substance are refused', async ({ page }) => {
    const errors = collectErrors(page);
    await page.goto('/#/chemistry-live-lab');
    await expect(page.getByTestId('chem-live-lab')).toBeVisible({ timeout: 30_000 });

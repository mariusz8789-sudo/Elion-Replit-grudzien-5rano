...
//
// HONEST FINDING (not a bug in this file — see the branch's handoff report, section S): the
// backend's governed-request hardening (campaign/labClosedLoop.mjs, branch
// claude/genesis-lab-closed-loop) now REQUIRES every validation request to carry either a real
// PRECLINICAL_CANDIDATE_PROTOCOL reference or an explicit `governedManualRequest` (reason +
// authorizedBy), and every endpointPlan entry that names a comparisonOutputKey to also carry a
// frozen `tolerance`. LabValidationPanel.tsx and its typed API client
// (packages/frontend/src/core/backend/client.ts) predate that hardening and send neither field —
// this is a real, current frontend/backend contract gap, not a flake. This test drives the REAL
// UI as far as it can honestly go and asserts the REAL resulting refusal, rather than silently
// working around it or pretending the full loop completed.
...
test.describe('Genesis Lab Closed Loop — CampaignScreen -> LabValidationPanel, real API, real candidate', () => {
  test('the panel renders over a real campaign candidate and honestly surfaces the real governed-request refusal', async ({ page }) => {
    ...
    // Drive the request-creation form for real.
    await page.locator('.account-form').filter({ hasText: 'Endpoint ID' }).locator('input').nth(1).fill('e2e-endpoint');
    const createButton = page.getByTestId('lab-validation-create-request');
    await expect(createButton).toBeEnabled();
    await createButton.click();

    // HONEST OUTCOME: the real backend refuses a governance-basis-free request (see this file's
    // header comment and the branch handoff report's frontend-bindings section). The panel must
    // show that real refusal, and the dossier must NOT gain a request — never a fabricated
    // "request created" success for a call that the server actually rejected.
    await expect(page.getByRole('status')).toBeVisible({ timeout: 10_000 });
    const statusText = await page.getByRole('status').textContent();
    expect(statusText ?? '', 'the panel must show the real backend refusal message, not a silently swallowed error').not.toMatch(/^Validation request:/);

    const counts = page.getByTestId('lab-validation-counts');
    await expect(counts).toBeVisible();
    await expect(counts).toContainText('0 request(s)');

    // No JS errors from driving the real UI this far — a real, intentional 400 network response
    // (the browser's own resource-load log line for the governed-request refusal above) is
    // expected and filtered out here; anything else is a genuine unexpected error. The whole
    // point of this product: never a claim anywhere on screen that a physical laboratory
    // executed anything.
    const unexpectedErrors = errors.filter((e) => !e.includes('400 (Bad Request)'));

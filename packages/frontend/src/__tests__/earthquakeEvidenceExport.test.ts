...
describe('Earthquake local evidence export', () => {
  it('exports the real ready envelope, evidence, replay, explicit mapping and honest labels deterministically', async () => {
    ...
    expect(payload).toMatchObject({
      ...
      mapping: {
        ...
        sourceCoordinateSystem: 'earthquake-synthetic-fixture-km/v1',
        targetCoordinateSystem: 'cityworld-layout-900x620/v1',
      },
      overlayGate: { enabled: true },
    });
    const serialized = serializeEarthquakeEvidenceExport(execution);
    expect(serialized).toBe(serializeEarthquakeEvidenceExport(execution));
  ...
  it('exports a named blocked record with no mapping or overlay', async () => {
    ...
    expect(parsed).toEqual(buildEarthquakeEvidenceExport(execution));
    expect(parsed).not.toHaveProperty('sourceArtifact');
    expect(parsed).not.toHaveProperty('hazardInput');
    expect(parsed).not.toHaveProperty('hazardRun');

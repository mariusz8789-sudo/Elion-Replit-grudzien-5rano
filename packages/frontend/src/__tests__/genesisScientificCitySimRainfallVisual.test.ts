...
describe('GenesisScientificCitySim — SPRINT C-2 rainfall rendering response', () => {
  ...
  it('this is a rendering response to a real event, not a new weather solver: the intensity parameter is still honestly refused', () => {
    const { sim } = buildInitializedSim();
    sim.triggerRainfallScenario();
    expect(sim.getRainfallCounterfactualGap()).toMatch(/NOT_MODELLED/);
  });
});

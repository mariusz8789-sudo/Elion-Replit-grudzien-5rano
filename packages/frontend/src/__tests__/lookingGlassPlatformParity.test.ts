 ...
 *
 * The three sessions below share no physics, no units, and no geometry — an
 * epidemic across a city, a growth curve in a bioreactor, and a molecule
 * decaying under a real C3 TemporalEngine are about as different as Genesis
 * worlds get; the third does not even come from the same ENGINE FAMILY as
 * the first two (scenarioEngine/hypothesisLoop vs core/worldModel/*). If the
 * SAME functions, called on all three, produce structurally consistent
 * answers, that is not a coincidence of similar implementations — it is one
 * implementation, proven across an engine boundary and not just a domain one.
 *
...
describe('Looking Glass — platform parity between city, laboratory and the real C3 chemistry world', () => {
  ...
  const domains = [city, lab, chemistry];

  it('all three resolve READY through the identical resolver, not a domain-specific one', () => {
    expect(city.resolution.status).toBe('READY');
    expect(lab.resolution.status).toBe('READY');
    expect(chemistry.resolution.status).toBe('READY');
    expect(new Set(domains.map((s) => s.request.family)).size).toBe(3);
  });

  it('all three worlds are instances of the SAME clock class, with the same contract', () => {
    ...
    expect(new Set(domains.map((s) => s.world!.clock.count)).size).toBe(3);
  ...
  it('all three event streams satisfy the identical InspectableEvent shape', () => {
    for (const session of domains) {
      ...
      for (const event of events) {
        ...
        // location/severity are honestly absent across all three real
        // domains here — none has spatial or graded events — and all three
        // say so the same way.
  ...
  it('all three shot plans and experience timelines come from the same builder functions', () => {
    for (const session of domains) {
  ...
  it('directionForFrame resolves a valid direction for all three, through the same function', () => {
    for (const session of domains) {
      ...
      expect(JSON.stringify(direction.cameraRequest)).not.toMatch(/epidemi|cell|infection|hypothesis|chemistry|kinetic/i);
    }
  });

  it('replay follows the identical MATCH-only rule across all three domains', () => {
    for (const session of domains) {
      for (const event of session.world!.getInspectableEvents()) {
  ...
  it('comparison, when requested, is computed by three DIFFERENT engines but reported through one shape', () => {
    ...
    for (const session of [comparedCity, comparedLab, comparedChemistry]) {
    ...
    // Three different engines, visible in provenance — parity is in the
    // SHAPE, not in pretending the domains compute comparison identically.
  ...
  it('chemistry has no renderer yet and says so through the SAME gate, not a domain-specific rendering hack', () => {
    // buildEpidemicSession/buildLaboratorySession set worldRoute for a real
    // 3D screen; buildChemistrySession honestly sets it null because no such
    // screen exists yet. enterWorld() checks `built.worldRoute === null` the
    // identical way regardless of which of the three built the session.
    expect(chemistry.worldRoute).toBeNull();
    expect(chemistry.enterWorld()).toBe(false);
  });
  ...
  it('describeEntityMoment exists identically on every session, real only where a live TemporalEngine backs it', () => {
    ...
    expect(chemistry.describeEntityMoment(1)).not.toBeNull();
  });
});

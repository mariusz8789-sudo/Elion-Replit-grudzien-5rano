...
const ALLOWED_ORPHANS: Readonly<Record<string, string>> = {
  ...
  'core/agent/evidenceProvenance.ts': 'Mandate step 4 (docs/DECISIONS.md D-042/D-043): the source-study-identity + evidence-class contract, deliberately landed BEFORE anything consumes it because the mandate forbids combining the ingest with the re-adjudication that acts on it. Evidence class is COMPUTED from the two studies being compared (same randomised study -> DIRECT_RANDOMISED, different studies -> INDIRECT_RANDOMISED, any non-randomised source -> OBSERVATIONAL), mirroring discoveryContracts.ts::classifyDiscoveryStatus; assertVetoEvidenceIsStrongest is the gate that refuses a veto drawn from weaker evidence while stronger evidence exists. 25/25 tests. Its real caller is the GOV-DRUG-CAMPAIGN-01 re-adjudication (mandate step 8, not yet built). Remove this entry once that wires it in.',
  'core/biotechData/surpass2DirectEvidence.ts': 'Mandate step 3: exposes the three tirzepatide arms of SURPASS-2 (NCT03987919) that were already pinned in this repository but unreachable as candidate-side observations, which is the structural cause of the D-042 defect (the campaign compared an n=16 cohort from NCT03322631 against this trial\'s semaglutide arm and vetoed on it). No new data was fetched -- clinicaltrials.gov is unreachable here -- only access to existing bytes was opened, with the declared sha256 recomputed from the file on disk by its own test. 15/15 tests. Same future caller as core/agent/evidenceProvenance.ts above.',

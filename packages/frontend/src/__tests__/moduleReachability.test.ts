...
const ALLOWED_ORPHANS: Readonly<Record<string, string>> = {
  ...
  'core/agent/falsifiedModelRegistry.ts': 'Discovery engine M2: the append-only cross-campaign index of models real observations already falsified, consulted before any model is emitted. Imported by core/agent/discoveryCampaign.ts, so it shares that module\'s Node-side reachability through core/repro/reproEntry.node.ts -> scripts/repro-demo.mjs; no browser screen renders a campaign yet.',

...
const ALLOWED_ORPHANS: Readonly<Record<string, string>> = {
  ...
  'core/agent/differentiatingExperimentGenerator.ts': 'Phase G, G2 (the mandate\'s own "sedno" -- the single piece both C1\'s own audit and the Qwen review independently flagged as the most concrete missing mechanism): UNRESOLVED_SURVIVORS -> a real discriminating experiment or an M1 gap request, never a bare stop. Extends observationGap.ts\'s existing 1-sigma discriminability discipline (TAU_DISCRIMINABILITY imported, not redefined) rather than inventing a second statistic; selection is by falsificationPower (fraction of hypothesis pairs actually separated), never gated on a single worst-case pair, so an experiment that separates 2 of 3 pairs is still chosen over none. Passes the mandate\'s own DD-EXP fixture (H1/H2 differ on available X, H2/H3 only on unavailable Y -> selects X, honestly reports H2-vs-H3 unresolved, raises a real ObservationGapRequest for Y in the same call). Decision rule frozen (fingerprinted) before any real observation. 9/9 tests. Its intended caller is the Krok 4 (Phase G) long-horizon campaign policy, not yet built -- reached today only by its own tests.',

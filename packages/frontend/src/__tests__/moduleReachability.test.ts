...
const ALLOWED_ORPHANS: Readonly<Record<string, string>> = {
  ...
  // --- Overnight Science Task 8: Pain Discovery -------------------------------
  'core/experimentFabric/painResearch.ts': 'Overnight Science Task 8: honest research-only Pain Discovery use case over the existing Human Digital Twin + Experiment Planner + Evidence + Meta-Cognition stack (runScientificIntegrationCampaign, unmodified). Returns BLOCKED by default (no pain-specific model is declared in HYPOTHESIS_PROBLEMS) or an explicitly-disclaimed PARTIAL against a caller-supplied stand-in problem -- never relabels another solver as pain research. This task explicitly forbids editing experimentFabric/index.ts ("report required exports to Codex in the handoff" instead), so production wiring (the index.ts export list, and any screen/campaign caller that supplies a real backingProblemId) is a deliberate decision left to Codex, not a side effect of landing this module. Reached today by finalPainResearch.test.ts. Remove this entry once Codex wires the export.',

  // --- Overnight Science Task 9: Spacetime/Temporal Scientific Integrity -----
  'core/experimentFabric/spacetimeIntegrity.ts': 'Overnight Science Task 9: validation-only physics-claim labeling over the existing canonical EpistemicLabel taxonomy (scientificWorlds/humanLab/epistemic.ts, unmodified) and the existing TemporalEngine/worldCounterfactual substrate (core/worldModel/temporal/temporalEngine.ts, core/worldModel/discovery/worldCounterfactual.ts, both read-only, neither modified, no second temporal engine). Implements NO renderer and NO world template, per this task\'s explicit scope. Same index.ts constraint as painResearch.ts above ("do not modify experimentFabric/index.ts") -- production wiring (which screen/campaign caller attaches a PhysicsClaim and reads validatePhysicsClaim\'s verdict) is deferred to Codex. Reached today by finalSpacetimeIntegrity.test.ts. Remove this entry once Codex wires the export.',
};

...
const ALLOWED_ORPHANS: Readonly<Record<string, string>> = {
  ...
  'core/agent/discoveryGraph.ts': 'Discovery engine §5: a campaign\'s reasoning as a lineage-preserving graph, and the only sanctioned path for knowledge to move BETWEEN campaigns (status preserved, never upgraded; a falsified model needs a declared assumption change). Reached at runtime through core/repro/reproEntry.node.ts -> scripts/repro-demo.mjs; no browser screen renders a campaign graph yet.',
  ...
  'core/agent/noveltyGate.ts': 'Phase E, Krok 1/7 (E2 Novelty Gate — the crux, built first per the mandate\'s own dependency order): assessNovelty/classifyResultLabel/assertValidResultLabel, TDD-proven against TE6 negatives (noveltyGate.test.ts, 11/11 passing incl. known-finding->DISCOVERY, NO_ACCESS->DISCOVERY, and BLOCKed-model->DISCOVERY all refused). Its real caller is the Krok 5 E3 Autonomous Campaign Orchestrator (not yet built) — every Phase E result label passes through this gate before that orchestrator writes it anywhere. Remove this entry once Krok 5 wires it in.',
  'core/agent/directionFinder.ts': 'Phase E, Krok 2/7 (E1 Open-Ended Direction Finder): findNextDirections reads a finished CampaignResult and proposes what to look at next (observation-gap follow-up, unresolved survivors, unexplained residual structure, cross-campaign transfer refusals) — grounded strictly in facts the campaign itself raised, same discipline as nextQuestion.ts (never invents a subject). Proven against real campaigns (makeQe4CampaignLab/makeKeplerCampaignLab) and synthetic ones, 7/7 tests, incl. the acceptance criterion generatedDirection != seededQuestion. Its real caller is the Krok 5 E3 orchestrator, which will call it after a campaign stops to seed the NEXT campaign without a human. Remove this entry once Krok 5 wires it in.',
  'core/agent/experimentFulfillment.ts': 'Phase E, Krok 3/7 (E5 Self-Directed Experiment Fulfillment): extends observationGap.ts (M1) + datasetLaboratory.ts with the mechanical steps M1 deliberately stops short of — find a qualified pinned source (exact point-grid match only), fetch, verify determinism with a second independent run, chain-of-custody, then resumeCampaignWithFulfilment reruns the real runDiscoveryCampaign engine and reports whether the verdict changed. NO_ACCESS_DECLARED (never a fabricated observation) whenever no registered source has the exact point. Proven against real pinned QE4 data (qe4DatasetLaboratory.ts), 6/6 tests. Its real caller is the Krok 5 E3 orchestrator, which will call it when a campaign it is running raises an ObservationGapRequest. Remove this entry once Krok 5 wires it in.',
  'core/agent/domainAdapter.ts': 'Phase E, Krok 4/7 (E4 Cross-Domain Execution): the shared DomainAdapter contract the audit found missing (item 14 — no existing adapter shape is shared across domains). A thin wrapper over the existing CampaignLaboratory/runDiscoveryCampaign (no second engine): capabilities/availableData/runExperiment/observe/provenance/replay/limitations. meetsProductionContract only requires >=2 adapters, names no domain. Proven with a real QE4+Kepler registry PLUS a hand-built third synthetic adapter (proving the contract is not secretly hardcoded to 2 domains), 7/7 tests. Its real caller is the Krok 5 E3 orchestrator. Remove this entry once Krok 5 wires it in.',
  ...
  'core/agent/campaignOrchestrator.ts': 'Phase E, Krok 5/7 (E3 Autonomous Campaign Orchestrator — the capstone): composes E1 (directionFinder.ts) + E2 (noveltyGate.ts) + E4 (domainAdapter.ts) + E5 (experimentFulfillment.ts) + the unmodified discoveryCampaign.ts engine into a loop that runs the next campaign without a human providing the question. Proven with a REAL multi-round autonomy trace (a frozen-grammar QE4-shaped residual drives an autonomous second campaign that finds the true model, autonomyProven=true), plus honest stopping rules on real Kepler (NO_INFORMATION_GAIN) and QE4 (INSUFFICIENT_DATA, no gapResolver) seeds, and the TE5 mandate fixture itself (Kepler seed -> REPRODUCTION, not a fabricated DISCOVERY). Reached at runtime through scripts/te5-autonomous-discovery-demonstrator.mjs and scripts/te7-multilingual-rtl-demo.mjs (Node-side, same non-browser boundary as qe4RegimeInquiryLoop.ts/structuralDiscovery.ts above) — no browser screen renders a Phase E campaign yet.',
  'core/agent/phaseELabels.ts': 'Phase E, Krok 7/7 (E6 Multilingual, Rule 1 canonical-key display layer): renderLabel/checkCompleteness over the exact canonical union Phase E itself produces (ResultLabel/NoveltyLevel/OrchestratorStopReason/DirectionGenerationMethod, imported as types only — TypeScript refuses to compile an incomplete dictionary). Proven: TE7.1 (one real run rendered in EN/PL/AR keeps the identical campaignFingerprint/resultLabel), TE7.4 (missing translation -> explicit flagged fallback, never silent), TE7.5 (19/19 canonical keys complete in all 3 locales), Rule 6 (every AR render flagged UNVERIFIED). 8/8 tests (phaseELabels.test.ts) plus scripts/te7-multilingual-rtl-demo.mjs (9/9, real Chromium RTL render). Reached today only by its own tests and that demonstrator — no browser screen uses it yet.',
  ...
  'core/agent/evidenceProvenance.ts': 'Mandate step 4 (docs/DECISIONS.md D-042/D-043/D-045): the source-study-identity + evidence-class contract. Evidence class is COMPUTED from the two studies being compared (same randomised study -> DIRECT_RANDOMISED, different studies -> INDIRECT_RANDOMISED, any non-randomised source -> OBSERVATIONAL), mirroring discoveryContracts.ts::classifyDiscoveryStatus. 25/25 tests. IMPORTANT, self-corrected in D-046: the SAFETY-VETO GATE actually used in production (a2OzempicSubstitute.ts::falsifyCandidate EVIDENCE_CLASS_GATED policy) does NOT call this module -- it reuses the pre-existing A2ComparisonType/comparisonType vocabulary already computed by extractCandidateSafety, avoiding a second decision engine. This file is transitively reached (via surpass2DirectEvidence.ts) only by core/biotechData/a2Surpass2ReAdjudication.ts and scripts/gov-drug-a2-surpass2-readjudication.mjs, which read SURPASS2_STUDY identity/hash but do not call classifyComparisonEvidenceClass/assertVetoEvidenceIsStrongest either. No production decision path calls this module\'s functions yet -- it remains a designed-ahead contract, not integrated logic. Kept documented here rather than removed so that stays visible.',
  ...
  'core/agent/genesisAdjudicationProtocol.ts': 'GENESIS ADJUDICATION PROTOCOL (D-047): the canonical, domain-agnostic phase machine (PRE_REGISTRATION -> FROZEN -> EXECUTED -> READJUDICATED -> COMPARED -> AUDITED) any Genesis re-adjudication follows. Two HARK guards (rule mutated after freeze; undeclared rule field changed between historical and re-adjudicated runs), both fail-closed. A reproducibility check runs the domain\'s decision function twice inside execute() and refuses to proceed on disagreement. Makes no scientific decision itself -- the domain supplies its own rule type, evidence records and runResult callback. 30/30 tests. Reached by core/biotechData/a2AdjudicationReferenceImplementation.ts (reference implementation #1) and scripts/genesis-adjudication-protocol-demo.mjs -- no browser screen yet.',
  ...
  'core/biotechData/govDrugLowerHarmPreregistration.ts': 'Mandate step 10 (LOWER-HARM), sealed BEFORE any candidate is re-ranked under it. New scenario id GOV-DRUG-DISCOVERY-E2E-02-LOWER-HARM, new fingerprint, deliberately NOT chained to E2E-01/campaign/A2 fingerprints (a new research question, not a re-analysis of data already observed). Reuses A2_MECHANISM_TARGETS/A2_REFERENCE_DRUG/A2_SAFETY_CATEGORIES/REFERENCE_HBA1C_DELTA_PP verbatim. Declares an efficacy floor (candidate must retain >=70% of the reference effect) as a hard gate evaluated before ranking, and a safety-dominant ranking rule distinct from A2\'s, machine-checked by assertSafetyDominatesRanking and assertNoNaturalnessBias (not just narrated). Every axis the mandate names is declared EVALUATED / DEFERRED_SEPARATE_WORK / NOT_CENTRAL_TO_DOMAIN with a rationale -- dependence/addiction axes are honestly marked NOT_CENTRAL_TO_DOMAIN for this GLP-1 candidate space rather than fabricated; the mandate\'s opioid-class demonstrator is the correct locus and is out of this seal\'s scope. 19/19 tests. Its real caller is the LOWER-HARM candidate generation/ranking pipeline, not yet built -- remove this entry once that wires it in.',
  'core/biotechData/govDrugLowerHarmRanking.ts': 'Mandate step 10, part 2: re-ranks runA2Analysis()\'s real, unmodified output under the sealed LOWER_HARM_PREREGISTRATION rule. No new candidate generated, no new datum fetched, no new veto invented -- reuses report.score.vetoed/vetoReason/safetyScore/efficacyScore/evidenceStrengthScore verbatim; only the efficacy-floor gate and safety-dominant ranking are new. Real run on the pinned candidate space: 3 of 12 candidates clear both the floor and the veto (native GLP-1, liraglutide, exenatide); verdict is CONFLICTING_EVIDENCE because no single candidate dominates both raw safety and raw efficacy-margin dimensions, even though the weighted composite clearly favours GLP-1 -- demonstrated, not forced. Discloses its own known limitation: inherits A2\'s HISTORICAL_NO_EVIDENCE_CLASS veto computation for every candidate; the D-046 evidence-class-gated re-adjudication is deliberately NOT retroactively applied here to avoid picking the rule that helps one candidate. 19/19 tests plus scripts/gov-drug-lower-harm-demonstrator.mjs (4/4 invariants). Reached only by its own test and that script -- no browser screen or funnel/TOP10/TOP2/falsification pipeline uses it yet, which is the next part of this mandate step.',
  'core/biotechData/govDrugLowerHarmFunnel.ts': 'Mandate step 10, part 3 (D-050): TOP10 -> TOP2 -> frozen falsification -> G2 -> adjudication -> comparison -> WINNER|NO_WINNER, completing the E2E. No second engine: reuses rankForLowerHarm (hard filter), generateDifferentiatingExperiment + TAU_DISCRIMINABILITY (G2, unmodified), buildFinalistPredictions (exported from govDrugDiscoveryCampaign.ts for this reuse), and evaluatePracticalCandidate/surfaceFor (the same safety/governance gate A2\'s own winner path uses). Real run on the real 12-candidate space: TOP2 = native GLP-1 + liraglutide (exenatide excluded, stated reason); G2 selects EFFICACY_DELTA_PP and fully separates the pair (100% falsification power); WINNER requires ALL THREE conjuncts (G2 separates the pair, the separation agrees with the frozen pre-experiment rank, the favoured candidate\'s own safety gate does not refuse it) -- real result is NO_WINNER on two independent, honest grounds: G2 favours the candidate that was NOT pre-rank #1, and BOTH TOP2 candidates independently fail EVIDENCE_SUFFICIENT (1 and 2 observations, minimum is 3) -- a genuine structural finding about this candidate space\'s evidence depth, not a bug. unresolvedContradictions in the gate are real G2 unresolvedPairs, never hardcoded []. 20/20 tests (real-run assertions plus synthetic-input coverage proving the WINNER path is genuinely reachable, since the real data does not itself produce one) plus scripts/gov-drug-lower-harm-funnel-demonstrator.mjs (5/5 invariants). Reached only by its own test and that script -- no browser screen uses it yet.',

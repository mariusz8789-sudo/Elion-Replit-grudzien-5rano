...
const ALLOWED_ORPHANS: Readonly<Record<string, string>> = {
  ...
  'core/agent/practicalCandidateGate.ts': 'Discovery engine §8: the machine-enforced gate a PracticalCandidate must pass before it may leave the research layer (evidence, provenance, replay, explicit epistemic status, declared limits, and the medical boundary enforced on the candidate\'s own output text). Reached at runtime through core/repro/reproEntry.node.ts -> scripts/repro-demo.mjs; no browser screen surfaces a candidate yet.',

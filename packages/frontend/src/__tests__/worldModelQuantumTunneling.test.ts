...
describe('The quantum lab is strictly opt-in', () => {
  it('the default flagship city has no junction and no quantum domain', () => {
    ...
    expect(city.base.availableDomains).toEqual(['chemistry-kinetics', 'electrical-engineering', 'epidemiology', 'hydraulics-engineering']);
  });

  it('opting in adds the quantum domain to the SAME world, alongside the existing four', () => {
    ...
    expect(city.base.availableDomains).toEqual(['chemistry-kinetics', 'electrical-engineering', 'epidemiology', 'hydraulics-engineering', 'quantum-mechanics']);
  });

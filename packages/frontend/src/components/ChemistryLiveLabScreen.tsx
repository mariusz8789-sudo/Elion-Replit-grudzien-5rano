...
/** The existing Chemistry Lab scene, driven by the current stage's model parameters. */
function LabVisual({ binding, stageParams }: { binding: ChemistryVisualBinding; stageParams: SimParams | undefined }) {
  ...
  return (
    <div className="cll-stage" data-testid="chem-visual">
      {def.createSim3D ? <Stage3D def={def} params={params} /> : <Stage2D def={def} params={params} />}
...
export function ChemistryLiveLabScreen() {
  ...
  return (
    <main className="cll" data-testid="chem-live-lab">
      <header className="cll-header">
        <div>
          <p className="cll-eyebrow">Chemistry Live Lab</p>
          <h1>Laboratorium chemii na żywo</h1>
        ...
        <section className="cll-col" aria-label="Eksperyment">
          ...
          {(run || executing || liveEvents.length > 0) && (
            <div className="cll-card cll-live" data-testid="chem-live">
              ...
              {template.visual && stages.length > 0 && <LabVisual binding={template.visual} stageParams={currentVisual} />}

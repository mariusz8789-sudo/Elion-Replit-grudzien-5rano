...
export function GenesisCapabilityShowcase() {
  ...
  const capabilities: Capability[] = [
    ...
    { id: 'hazard', label: 'Earthquake / Hazard', description: 'Hazard and exposure workflows where the existing model and data contract are present.', status: 'PARTIAL', action: 'Inspect hazard', hash: '#/earthquake', note: 'Model/data availability is scenario-dependent' },
    ...
    { id: 'physics', label: 'Physics', description: 'Existing physics labs and experiment-fabric routes, shown with their actual runtime status.', status: models.length > 0 ? 'AVAILABLE' : 'PARTIAL', action: 'Open physics', hash: '#/lab/physics', note: `${labs.length} registered labs · ${models.length} router models` },

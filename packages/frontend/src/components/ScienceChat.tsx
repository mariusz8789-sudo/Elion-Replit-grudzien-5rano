...
function formatNaturalDiscoveryResult(result: Awaited<ReturnType<typeof resolveNaturalFunctionalReplacementFromSources>>): string {
  ...
  return [`NATURAL DISCOVERY — ${result.status}`, result.reason, `Kandydaci/raporty: ${result.reports.length}.`, ...(combination ? [`COMBINATION HYPOTHESIS ${combination.combinationId}: ${combination.candidateIds.join(' + ')}`, `Target coverage: ${combination.coveredTargetIds.join(', ') || 'UNKNOWN'}; uncovered: ${combination.uncoveredTargetIds.join(', ') || 'none declared'}`, ...combination.validationPlan.map((step) => `NEXT VALIDATION: ${step}`)] : []), ...(compute.length ? ['CHEAP COMPUTE (existing Fabric):', ...compute] : []), ...(heavy.length ? ['HEAVY COMPUTE (existing backend Fabric):', ...heavy] : []), ...(admet.length ? ['ADMET-AI (MODEL_ESTIMATE):', ...admet] : []), ...top.map((report, index) => {

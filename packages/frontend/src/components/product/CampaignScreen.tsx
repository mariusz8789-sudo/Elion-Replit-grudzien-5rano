...
import {
  ...
  transitionMolecule, pendingMolecules, campaignCandidates,
...
export function CampaignScreen({ id }: { id: string }) {
  ...
  // `locale` in deps so the localized ranking reasons/verdicts recompute on language switch.
  const ranked = useMemo(() => (campaign ? rankCandidates(campaignCandidates(campaign)) : []), [campaign, locale]);
  const summary = useMemo(() => (campaign ? campaignSummary(campaign, ranked) : null), [campaign, ranked, locale]);
  useEffect(() => { if (!referenceId && ranked.length) setReferenceId(ranked[0].id); }, [ranked, referenceId]);

  if (!session) {
    return <ProductChrome active="#/campaigns"><Panel title={t('common.signIn')} icon="lock"><AccountPanel /></Panel></ProductChrome>;
  }
  if (!campaign) {
    return (
      <ProductChrome active="#/campaigns">
        <div className="ds-empty"><Icon name="briefcase" size={26} className="ds-empty-icon" /><h4>{t('cd.notFound.title')}</h4><p>{t('cd.notFound.body')}</p><a className="ds-btn ds-btn-primary" href="#/campaigns">← {t('cd.back')}</a></div>
      </ProductChrome>
    );
  }
  ...
  const exportCSV = () => downloadText(`${campaign.name || 'project'}.csv`, 'text/csv', campaignToCSV(campaign, exportMeta()));
  const exportJSON = () => downloadText(`${campaign.name || 'project'}.json`, 'application/json', campaignToJSON(campaign, exportMeta()));
  ...
  const runLabel = campaign.molecules.some((m) => m.status !== 'PENDING') ? t('cd.runResume') : t('cd.run');

  const desktopBody = (
    <>
      {/* Header + controls */}
      <Panel title={campaign.name} icon="briefcase" right={<a className="ds-chip" href="#/campaigns"><Icon name="back" size={13} /> {t('cd.back')}</a>}>
        <p className="ds-note" style={{ marginTop: 0 }}><strong>{t('cd.goal')}</strong> {campaign.goal || '—'} · <span className="ds-dim">{campaign.owner} · {fmtDate(campaign.createdAt)}</span></p>
        <textarea className="compare-input ds-mono" value={addRaw} onChange={(e) => setAddRaw(e.target.value)} rows={4} spellCheck={false} placeholder={t('cd.addPlaceholder')} />
        <div className="ds-input-row ds-mt">
          <button className="ds-btn" onClick={doAdd} disabled={!!run || !addRaw.trim()}><Icon name="spark" size={14} /> {t('cd.addMolecules')}</button>
          ...
          {run ? <button className="ds-btn" onClick={cancel}><Icon name="block" size={14} /> {t('cd.abort')}</button> : null}
          {ranked.length ? <>
            <button className="ds-btn" onClick={exportCSV}>{t('cd.exportCSV')}</button>
            <button className="ds-btn" onClick={exportJSON}>{t('cd.exportJSON')}</button>
            <button className="ds-btn" onClick={() => window.print()}>{t('cd.exportPDF')}</button>
          </> : null}
        </div>
        {(run || pendCount > 0) ? (
          <div className="run-status ds-mt">
            ...
            <div className="run-stats">
              <span>{t('cd.stat.total')}: <strong>{campaign.molecules.length}</strong></span>
              <span className="ok">{t('cd.stat.analysed')}: <strong>{campaign.molecules.filter((m) => m.status === 'ANALYSED').length}</strong></span>
              <span className="bad">{t('cd.stat.invalid')}: <strong>{campaign.molecules.filter((m) => m.status === 'INVALID').length}</strong></span>
              <span>{t('cd.stat.pending')}: <strong>{pendCount}</strong></span>
              {run ? <><span>{t('cd.stat.progress')}: <strong>{run.done}/{run.total}</strong></span><span>{t('cd.stat.eta')}: <strong>{etaText(run)}</strong></span></> : null}
            </div>
          </div>
        ) : null}
      </Panel>
      ...
      {ranked.length === 0 ? (
        <div className="ds-empty ds-mt"><Icon name="flask" size={24} className="ds-empty-icon" /><h4>{t('cd.empty.title')}</h4><p>{t('cd.empty.body')}</p></div>
      ) : null}

      {/* Dashboard */}
      {summary && ranked.length ? (
        <Panel title={t('cd.dashboard.title')} icon="chart" className="ds-mt" right={<StatusPill kind="info">{t('cd.dashboard.done', { pct: progressPct })}</StatusPill>}>
          <div className="dash-grid">
            <div className="dash-card dash-ok"><header><StatusPill kind="ok"><Icon name="check" size={12} /> {t('cd.card.continue')}</StatusPill><span className="dash-count">{summary.verdictCounts.CONTINUE}</span></header></div>
            <div className="dash-card"><header><StatusPill kind="warn"><Icon name="flask" size={12} /> {t('cd.card.needsValidation')}</StatusPill><span className="dash-count">{summary.needsValidation.length}</span></header></div>
            <div className="dash-card dash-blocked"><header><StatusPill kind="blocked"><Icon name="block" size={12} /> {t('cd.card.rejected')}</StatusPill><span className="dash-count">{summary.rejected.length}</span></header></div>
            <div className="dash-card"><header><StatusPill kind="info"><Icon name="alert" size={12} /> {t('cd.card.invalid')}</StatusPill><span className="dash-count">{summary.invalid}</span></header></div>
          </div>
          {summary.averages ? (
            <p className="ds-note ds-mt">{t('cd.averages', { count: summary.averages.count, mw: summary.averages.molWt.toFixed(0), logp: summary.averages.logP.toFixed(2), tpsa: summary.averages.tpsa.toFixed(0), hbd: summary.averages.hbd.toFixed(1), hba: summary.averages.hba.toFixed(1) })} {summary.grounding.note}</p>
          ) : null}
        </Panel>
      ) : null}
      ...
      {ranked.length ? (
        <Panel title={t('cd.rankTitle')} icon="chart" className="ds-mt" right={<StatusPill kind="info">{t('cd.rankEngine')}</StatusPill>}>
          <div className="ds-table-wrap">
            <table className="ds-table rank-table">
              <thead><tr><th>#</th><th>{t('cd.col.molecule')}</th><th>{t('cd.col.score')}</th><th>{t('cd.col.recommendation')}</th><th>{t('cd.col.why')}</th><th></th></tr></thead>
              <tbody>
                {ranked.map((c) => {
                  ...
                  return (
                    <Fragment key={c.id}>
                      <tr>
                        ...
                        <td><button className="decision-explain-btn" onClick={() => toggleExpand(c.id)}>{open ? t('cd.trace.hide') : t('cd.trace.show')}</button></td>
                      </tr>
      ...
      {campaign.molecules.length ? (
        <Panel title={t('cd.lifecycle')} icon="clock" className="ds-mt" right={<StatusPill kind="info">{t('cd.moleculesCount', { count: campaign.molecules.length })}</StatusPill>}>
          <div className="ds-table-wrap">
            <table className="ds-table">
              <thead><tr><th>{t('cd.col.molecule')}</th><th>{t('cd.col.status')}</th><th>{t('cd.col.stage')}</th><th>{t('cd.col.history')}</th><th>{t('cd.col.actions')}</th></tr></thead>
              <tbody>
                {campaign.molecules.map((m) => (
                  <tr key={m.id}>
                    ...
                    <td><StatusPill kind={m.status === 'ANALYSED' ? 'ok' : m.status === 'INVALID' ? 'blocked' : 'warn'}>{m.status === 'ANALYSED' ? t('mol.ok') : m.status === 'INVALID' ? t('mol.invalidSmiles') : t('mol.pending')}</StatusPill>{m.status === 'INVALID' && m.invalidReason ? <div className="ds-dim invalid-reason">{m.invalidReason}</div> : null}</td>
                    <td>{t(`stage.${m.stage}`)}</td>
                    <td className="timeline-cell">{m.timeline.map((tr, i) => <span key={i} className="tl-step">{t(`stage.${tr.to}`)}{i < m.timeline.length - 1 ? ' →' : ''}</span>)}</td>
                    <td className="stage-actions">{m.status === 'ANALYSED' ? STAGE_ACTIONS.map((a) => <button key={a.stage} className="ds-chip" disabled={m.stage === a.stage} onClick={() => setStage(m.id, a.stage)} title={t(`stageAction.${a.stage}`)}><Icon name={a.icon} size={12} /></button>) : <span className="ds-dim">—</span>}</td>
                  </tr>
                ))}
              </tbody>
            </table>
  ...
  const mobileBody = (
    <div className="cmp-mobile">
      <section className="cmp-hero">
        <div className="cmp-hero-eyebrow">{t('cd.eyebrow')}</div>
        <h2 className="cmp-hero-name">{campaign.name}</h2>
        <p className="cmp-hero-why">{campaign.goal || t('cd.noGoal')}</p>
        <div className="cmp-hero-tags">
          <span className="cmp-hero-tag">{campaign.owner}</span>
          <span className="cmp-hero-tag">{fmtDate(campaign.createdAt)}</span>
          <a className="cmp-hero-tag" href="#/campaigns"><Icon name="back" size={11} /> {t('cd.back')}</a>
        </div>
      </section>

      <section className="cmp-section">
        <h3 className="cmp-section-title">{t('cd.addMolecules')}</h3>
        <textarea className="compare-input ds-mono" value={addRaw} onChange={(e) => setAddRaw(e.target.value)} rows={4} spellCheck={false} placeholder={t('cd.addPlaceholderMobile')} />
        <div className="ds-input-row ds-mt">
          <button className="ds-btn" onClick={doAdd} disabled={!!run || !addRaw.trim()}><Icon name="spark" size={14} /> {t('cd.add')}</button>
          <MoleculeCsvImport disabled={!!run} onImport={(lines) => setAddRaw((prev) => (prev.trim() ? `${prev}\n${lines}` : lines))} />
          {run ? <button className="ds-btn" onClick={cancel}><Icon name="block" size={14} /> {t('cd.abort')}</button> : null}
        </div>
        {pendCount > 0 && !run ? (
      ...
      {ranked.length === 0 ? (
        <div className="ds-empty"><Icon name="flask" size={24} className="ds-empty-icon" /><h4>{t('cd.empty.title')}</h4><p>{t('cd.empty.body')}</p></div>
      ) : null}

      {summary && ranked.length ? (
        <>
          <div className="cmp-stat-strip">
            <span className="cmp-stat-strip-item"><StatusPill kind="ok">{summary.verdictCounts.CONTINUE}</StatusPill> {t('cd.card.continue')}</span>
            <span className="cmp-stat-strip-item"><StatusPill kind="warn">{summary.needsValidation.length}</StatusPill> {t('cd.card.needsValidation')}</span>
            <span className="cmp-stat-strip-item"><StatusPill kind="blocked">{summary.rejected.length}</StatusPill> {t('cd.card.rejected')}</span>
            <span className="cmp-stat-strip-item"><StatusPill kind="info">{summary.invalid}</StatusPill> {t('cd.card.invalid')}</span>
          </div>
      ...
      {campaign.molecules.length ? (
        <section className="cmp-section">
          <h3 className="cmp-section-title">{t('cd.lifecycle')}</h3>
          <div className="cmp-cand-list">
            {campaign.molecules.map((m) => (
              <div key={m.id} className="cmp-cand-card">
                ...
                <div className="cmp-cand-verdict">
                  <StatusPill kind={m.status === 'ANALYSED' ? 'ok' : m.status === 'INVALID' ? 'blocked' : 'warn'}>{m.status === 'ANALYSED' ? t(`stage.${m.stage}`) : m.status === 'INVALID' ? t('mol.invalidSmiles') : t('mol.pending')}</StatusPill>
                </div>
                {m.status === 'INVALID' && m.invalidReason ? <p className="ds-dim invalid-reason" style={{ marginTop: '0.4rem' }}>{m.invalidReason}</p> : null}
                {m.status === 'ANALYSED' ? (
                  <div className="ds-input-row ds-mt">
                    {STAGE_ACTIONS.map((a) => (
                      <button key={a.stage} className="ds-chip" disabled={m.stage === a.stage} onClick={() => setStage(m.id, a.stage)}><Icon name={a.icon} size={12} /> {t(`stageAction.${a.stage}`)}</button>
                    ))}
                  </div>
                ) : null}
              </div>

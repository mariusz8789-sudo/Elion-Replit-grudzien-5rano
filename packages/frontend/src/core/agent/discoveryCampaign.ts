  ...
  for (let round = 1; round <= maxRounds; round += 1) {
    ...
    if (unobserved.length > 0) {
      ...
      selectionReason = selectedNextX === null
        ? 'No candidate experiment produced a finite discrimination score.'
        : `Chose ${lab.xLabel}=${selectedNextX} because the ${fitted.length} live models' predictions disagree there by ${discriminationScore?.toFixed(3)}× the typical observation sigma — the widest disagreement among ${unobserved.length} unobserved candidate(s).`;
    }

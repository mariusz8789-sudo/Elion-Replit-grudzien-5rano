...
export type AnchorRunResult =
  | {
    ...
    /**
     * BELIEF REVISION (`core/experimentFabric/beliefRevision.ts`) — czy ten
     * jeden przebieg kotwicy faktycznie ZMIENIA stan przekonania o hipotezie
     * "model universe-kepler/cheminformatics poprawnie przewiduje ${metric}",
     * a nie tylko drukuje werdykt i kończy. Świeża hipoteza za każdym
     * wywołaniem (prior 0.5) — ten obiekt jest raportem z JEDNEGO przebiegu,
     * nie trwałym stanem przekonania w Science Memory (to zadanie wywołującego).
     * Rozmiar ruchu jest ograniczony przez `evidenceCeiling` Tautology Gate:
     * `CONSISTENCY_CHECK`/`UNTESTABLE` dają sufit 0 (żaden ruch), więc kotwica
     * bez prawdziwie niezależnej obserwacji nie mogłaby fałszywie zwiększyć
     * pewności.
     */
    readonly belief: { readonly before: number; readonly after: number; readonly status: Hypothesis['status'] };
    /** Konkretna, next-question propozycja — co warto sprawdzić dalej, w
     * zależności od werdyktu. Tekst, nie nowa logika: pętla PROBLEM → ... →
     * NEXT QUESTION wymaga, żeby coś było zaproponowane, a nie żeby demo się
     * kończyło na jednym werdykcie. */
...
export function runExternalAnchor(
  ...
  // Świeża hipoteza, prior 0,5 — patrz komentarz przy polu `belief` w
  // `AnchorRunResult`. Ruch ograniczony sufitem Bramy: dla CONSISTENCY_CHECK/
  // UNTESTABLE sufit wynosi 0, więc żaden werdykt nie mógłby przesunąć
  // przekonania bez prawdziwie niezależnej obserwacji.
  const beliefHypothesis = createHypothesis(`${anchor.id}:model-correct`, criterion, 0.5);
  const evidenceCap = evidenceCeiling(tautologyAssessment.classification);
  const rawMagnitude =
    verification.predictedValue !== null && verification.observedValue !== null
      ? evidenceMagnitudeWithinTolerance(verification.observedValue, verification.predictedValue, anchor.tolerance)
      : 0;
  const evidenceMagnitude = evidenceCap === null ? rawMagnitude : Math.min(rawMagnitude, evidenceCap);
  const revisedBelief = updateConfidence(
    beliefHypothesis,
    verification.assessment,
    evidenceMagnitude,
    `Kotwica "${anchor.id}": ${verification.assessment} (Tautology Gate: ${tautologyAssessment.classification}).`,
    0,
  );

  const nextQuestion =
    verification.assessment === 'SUPPORTED_WITHIN_PROTOCOL'
      ? `This held for ${anchor.id} — the natural next test is a case that strains this anchor's own stated boundary (see "what this does NOT establish" above), not another easy confirmation of the same regime.`
      : verification.assessment === 'FALSIFIED_WITHIN_PROTOCOL'
        ? `${anchor.id} was FALSIFIED within the preregistered tolerance — the next question is WHY: re-check the unit conversion and constant used, then check whether the tolerance itself was too tight before concluding the underlying model is wrong.`
        : `${anchor.id} was INCONCLUSIVE — resolve that first (missing/ambiguous data in the pinned payload) before drawing any conclusion about the model.`;
  ...
  return {
    ...
    tautologyAssessment,
    belief: { before: beliefHypothesis.confidence, after: revisedBelief.confidence, status: revisedBelief.status },
    nextQuestion,
  };
}

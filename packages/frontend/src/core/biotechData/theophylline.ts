...
export function buildPinnedChEMBLTheophyllineDiscovery(): ChEMBLTheophyllineDiscovery {
  ...
  const safety: SafetySignal = { kind: 'safety-signal', id: `safety:unknown:${compoundId}`, namespace: 'genesis-biotech', label: 'Theophylline safety evidence not assessed in this workflow', status: 'UNKNOWN', signalType: 'uncertainty', description: 'The pinned ChEMBL binding record does not provide a safety or toxicity assessment for this discovery report.', evidenceQuality: 'UNKNOWN', uncertainty: 'No safety/ADME/Tox source was incorporated for this candidate.', provenance: [] };
  const candidate: TherapeuticCandidate = { kind: 'therapeutic-candidate', id: `candidate:${compoundId}:${targetId}`, namespace: 'genesis-biotech', label: 'Theophylline — A1 binding research candidate', status: 'UNKNOWN', materialId: compoundId, compoundIds: [compoundId], targetIds: [targetId], mechanismIds: [], supportingEvidenceIds: [activityId], safetySignalIds: [safety.id], hypothesisIds: [`hypothesis:candidate:${compoundId}:${targetId}`], provenance: [provenance] };

...
export interface EarthquakeDemoEnvelope {
  ...
  readonly scenario: EarthquakeScenarioResult | null;
...
export async function buildEarthquakeDemoEnvelope(
  ...
  try {
    ...
    if (err instanceof ImmutableConflictError) {
      ...
      // the same scenarioLabel re-run with different parameters. This is a real immutable
      // provenance conflict, distinct from registry/module compatibility admission.
      return blocked('PROVENANCE_CONFLICT', `scenario id conflict: ${err.message}`, {
        moduleDescriptor: descriptor, scenario: result, run: result.run,
      });
    }
    throw err;
  }
  ...
  try {
    ...
    return blocked('REGISTRY_INCOMPATIBLE', message, { moduleDescriptor: descriptor, scenario: result, run: result.run });
  }
  ...
  if (evidencePack.missingFields.length > 0) {
    return blocked('EVIDENCE_INCOMPLETE', `evidence pack missing: ${evidencePack.missingFields.join(', ')}`, {
      moduleDescriptor: descriptor, scenario: result, run: result.run, evidence: evidencePack,
    });
  }

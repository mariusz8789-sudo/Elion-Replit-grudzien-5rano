...
const DOMAIN_REGISTRY: readonly KnowledgeDomainDescriptor[] = [
  ...
  },
  {
    ...
    // tutaj, a Scientific Discovery Layer projektuje protokoły tylko dla
    // modeli zadeklarowanych w rejestrze. To dopisanie istniejącego faktu,
  ...
  },
  {
    ...
    capability: 'REAL_ENGINE', realModels: ['chemistry-arrhenius', 'chemistry-ising', 'chemistry-titration', 'chem-vsepr', 'chem-molecular-weight', 'chem-rdkit-descriptors', 'quantum-chemistry-pyscf-h2-rhf'],
    concepts: ['Arrhenius', 'VSEPR', 'miareczkowanie', 'Ising', 'magnetyzacja spontaniczna', 'masa molowa', 'RHF', 'H₂', 'basis set', 'single-point energy'],
    parameters: ['temperatureK', 'activationEnergyKJ', 'temperature', 'seed', 'acid', 'vb', 'shapeId', 'formula', 'smiles', 'bondLengthAngstrom', 'basis'], units: ['K', 'kJ/mol', 'J/k_B', 'g/mol', 'Å', 'Hartree'],
    assumptions: ['Ising używa 2D siatki kwadratowej J=1, najbliższych sąsiadów, periodycznych brzegów oraz seedowanego Metropolisa; dokładna magnetyzacja jest referencją granicy termodynamicznej, a pojedynczy run na 42×42 nie jest miarą niepewności. Miareczkowanie rozwiązuje bilans ładunku czterech lokalnych słabych kwasów przy ustalonych Ca=Cb=0,1 mol/L i Va=25 mL, więc nie jest pomiarem próbki. VSEPR zwraca istniejące wektory geometrii domen elektronowych; dla NH₃ i H₂O używa zdefiniowanych kątów zmierzonych, a dla pozostałych geometrii z wolnymi parami nie estymuje indywidualnych odchyleń. PySCF wykonuje wyłącznie ograniczony neutralny singlet H₂ RHF single-point w bazie sto-3g albo 6-31g; nie jest to optymalizacja geometrii, pełna chemia kwantowa ani predykcja eksperymentalna. Molecular dynamics i docking wymagają odrębnych silników.'],
    possibleExperiments: ['oblicz kinetykę reakcji', 'uruchom 2D model Isinga', 'oblicz krzywą miareczkowania', 'pokaż geometrię VSEPR', 'oblicz masę molową', 'analizuj cząsteczkę', 'porównaj RHF/STO-3G i RHF/6-31G dla tej samej geometrii'], requiredSolver: 'ModelGraph / Ising Metropolis / VSEPR geometry / RDKit capability / PySCF 2.14.0 przez GENESIS_PYSCF_PYTHON',
  ...
  },
  {
    ...
    capability: 'CAPABILITY_SEAM', realModels: ['quantum-bloch', 'quantum-bloch-circuit', 'quantum-teleportation', 'quantum-tunneling-1d', 'chsh-correlation', 'quantum-chsh-correlation', 'quantum-kitaev-bulk', 'quantum-entanglement-measures'],
    concepts: ['superpozycja', 'interferencja', 'splątanie', 'teleportacja kwantowa', 'tunelowanie', 'kubit', 'bramki', 'bulk BdG', 'łańcuch Kitaeva'], parameters: ['theta', 'phi', 'circuit', 'state', 'a', 'aP', 'b', 'bP', 'energy', 'barrier', 'width', 'chemicalPotential', 'hopping', 'pairing', 'stateId', 'familyParameter', 'mixingAngleDeg', 'whiteNoise'], units: ['rad', 'jedn. energii', 'nat', 'bit', 'deg'],
    ...
    possibleExperiments: ['wykonaj obwód jednokubitowy Blocha', 'zweryfikuj teleportację kwantową', 'oblicz analityczną korelację CHSH', 'pokaż stan kubitu', 'zbadaj korelację CHSH', 'oblicz bulk gap łańcucha Kitaeva', 'policz miary splątania stanu', 'sprawdź granicę Tsirelsona', 'sprawdź monogamię splątania', 'sprawdź czy PPT wystarcza'], requiredSolver: 'single-qubit matrices / three-qubit quantumState / Schrödinger solver required for tunneling; Q1 Kitaev bulk dostępny dla zdefiniowanego modelu',
    visualization: ['numeric', 'graph', 'scene-3d'], dependencies: ['mathematics', 'electrodynamics'], keywords: ['kwant', 'kubit', 'bloch', 'bramka', 'obwód kwantowy', 'teleportacja kwantowa', 'splątanie', 'splatanie', 'tunelowanie', 'chsh', 'kitaev', 'majorana', 'no-cloning', 'zakaz klonowania', 'no-communication', 'brak sygnalizacji'],
  },
  {
  ...
  },
  {
    ...
    visualization: ['numeric', 'graph', 'canvas-2d', 'scene-3d'], dependencies: ['classical-mechanics', 'universe'], keywords: ['czarna dziura', 'schwarzschild', 'czasoprzestrzeń', 'czasoprzestrzen', 'dylatacja', 'einstein', 'grawitacyj', 'tunel czasoprzestrzenny', 'wormhole', 'most einsteina-rosena', 'er=epr', 'warp', 'alcubierre'],
  },
  {

...
const DOMAIN_REGISTRY: readonly KnowledgeDomainDescriptor[] = [
  ...
  },
  {
    ...
    concepts: ['Arrhenius', 'VSEPR', 'miareczkowanie', 'Ising', 'magnetyzacja spontaniczna', 'masa molowa', 'PySCF', 'RHF', 'STO-3G', 'energia elektronowa H₂'],
    parameters: ['temperatureK', 'activationEnergyKJ', 'temperature', 'seed', 'acid', 'vb', 'shapeId', 'formula', 'smiles', 'bondLengthAngstrom'], units: ['K', 'kJ/mol', 'J/k_B', 'g/mol', 'Å', 'Hartree'],
    assumptions: ['Ising używa 2D siatki kwadratowej J=1, najbliższych sąsiadów, periodycznych brzegów oraz seedowanego Metropolisa; dokładna magnetyzacja jest referencją granicy termodynamicznej, a pojedynczy run na 42×42 nie jest miarą niepewności. Miareczkowanie rozwiązuje bilans ładunku czterech lokalnych słabych kwasów przy ustalonych Ca=Cb=0,1 mol/L i Va=25 mL, więc nie jest pomiarem próbki. VSEPR zwraca istniejące wektory geometrii domen elektronowych; dla NH₃ i H₂O używa zdefiniowanych kątów zmierzonych, a dla pozostałych geometrii z wolnymi parami nie estymuje indywidualnych odchyleń. Molecular dynamics, docking i quantum chemistry wymagają zewnętrznych silników.', 'Dostępny PySCF wykonuje wyłącznie deterministyczny single-point H₂: neutralny singlet, liniowa geometria, RHF/STO-3G i jawna długość wiązania. Energia jest wynikiem ograniczonego modelu obliczeniowego, nie eksperymentalnym pomiarem, pełną krzywą dysocjacji, predykcją materiału ani walidacją innej metody lub bazy.'],
    possibleExperiments: ['oblicz kinetykę reakcji', 'uruchom 2D model Isinga', 'oblicz krzywą miareczkowania', 'pokaż geometrię VSEPR', 'oblicz masę molową', 'analizuj cząsteczkę', 'uruchom PySCF H₂ RHF/STO-3G dla jawnej długości wiązania'], requiredSolver: 'ModelGraph / Ising Metropolis / VSEPR geometry / RDKit capability / PySCF H₂ RHF/STO-3G single-point',
  ...
  },
  {
    ...
    parameters: ['antigen_sequence', 'antibody_id', 'membrane_context', 'measurement_type', 'referencePdb', 'mobilePdb'],
    units: ['nM', 'Å', 's⁻¹'],
    assumptions: [
    ...
    possibleExperiments: [
      ...
      'porównaj publiczne struktury HIV MPER / 10E8 PDB przez Cα RMSD',

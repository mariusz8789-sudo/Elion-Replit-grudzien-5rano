...
const KIND_UNSUPPORTED_REASON: Readonly<Partial<Record<ScenarioKind, string>>> = {
  ...
  WILDFIRE:
    'a real fuel-bed spread model exists (core/worldModel/domains/wildfireSpread.ts — a real Rothermel (1972) surface-fire-spread model, a real elliptical fire-shape model, and real minimum-travel-time Dijkstra propagation over the same terrain floodInundation.ts uses) but it has no standalone Looking Glass binding to route to yet. Crown fire, spotting (ember lofting — a major real driver this omits), fire-weather coupling, weather-derived fuel moisture and suppression genuinely remain unmodelled regardless of routing, and only 4 of Anderson\'s 13 standard fuel models are implemented.',
  EARTHQUAKE:
  ...
  LANDSLIDE:
    'a real slope-stability and runout model exists (core/worldModel/domains/landslide.ts — infinite-slope stability analysis (Skempton & DeLory 1957) giving a real per-cell factor of safety from the same terrain floodInundation.ts and wildfireSpread.ts use, and a physically-derived sliding-block energy balance tracing runout velocity downhill by D8 steepest descent) but it has no standalone Looking Glass binding to route to yet. Geotechnical parameters (cohesion, friction angle, saturation) are stated literature values rather than measured, and debris-flow rheology, mass deformation, erosion/entrainment and any flow-extent or impact-pressure field genuinely remain unmodelled regardless of routing — this traces a runout centreline and velocity, not a debris-flow footprint.',
  DROUGHT:
    'a real water-balance model exists (core/worldModel/domains/drought.ts — the Thornthwaite-Mather (1955) soil-moisture bucket model: real accounting of precipitation vs. evapotranspiration vs. runoff vs. stored soil moisture) but it has no standalone Looking Glass binding to route to yet. This is not the Standardized Precipitation Index, SPEI, or Palmer Drought Severity Index — those need decades of real climatological records Genesis has for nowhere — so severity uses absolute soil-moisture bands, not a calibrated percentile, regardless of routing.',
  BLACKOUT:

...
export function createHumanDigitalTwinManifest(twinId = `HDT-${stableHash(Date.now())}`, parameters = defaultHumanParameters()): HumanDigitalTwinManifest {
  ...
  const organs: AnatomyNode[] = [
    ...
    // The one major vessel of the BodyParts3D pilot (FMA3734). Its proxy is a thin column along the atlas
    // midline; with the approved reference mesh loaded, the twin draws the aorta's real shape instead.
    node({ id: 'aorta', parentId: 'thorax', kind: 'ORGAN', label: 'Aorta', latinLabel: 'Aorta', system: 'CARDIOVASCULAR', scaleMeters: 0.36, positionMeters: P(-0.01, 1.2, 0.02), dimensionsMeters: D(0.035, 0.36, 0.035), assetSlot: 'human.organ.aorta.high_fidelity.glb', visibleByDefault: false, epistemic: 'MODEL' }),

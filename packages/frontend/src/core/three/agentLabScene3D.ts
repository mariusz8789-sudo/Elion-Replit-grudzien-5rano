...
export class AgentLabScene3D implements Sim3D {
  ...
  syncScene(_scene: THREE_NS.Scene, camera: THREE_NS.PerspectiveCamera): void {
    ...
    if (this.cameraMode === 'VISOR') {
      ...
      const height = macroVisible ? 1.58 : tight ? 1.45 : 1.4;
      ...
      // The existing macro layer becomes the subject of the shot; it is not hidden behind a dock.
      ...
      this.scratchB.set(TWIN_CHAMBER.position.x + panelOffset, macroVisible ? 1.42 : portrait ? 1.03 : 1.08, TWIN_CHAMBER.position.z);

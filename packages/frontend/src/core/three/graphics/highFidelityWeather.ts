...
export function weatherProfile(weather?: string): WeatherProfile {
  switch ((weather ?? 'CLEAR').toUpperCase()) {
    case 'RAIN': return { density: 2600, speed: 16, color: 0xcfe8ff, size: 0.032, opacity: 0.58, fogDensity: 0.018 };
    case 'STORM': return { density: 4200, speed: 24, color: 0xaec9e8, size: 0.045, opacity: 0.64, fogDensity: 0.028 };
    case 'SNOW': return { density: 1600, speed: 1.5, color: 0xffffff, size: 0.06, opacity: 0.78, fogDensity: 0.014 };
...
export function createHighFidelityWeatherRig(THREE: typeof THREE_NS, scene: THREE_NS.Scene, weather?: string): WeatherRig {
  ...
  group.name = `genesis-weather-${(weather ?? 'CLEAR').toLowerCase()}`;
  scene.add(group);
  ...
  const positions = new Float32Array(profile.density * 3);
  const velocities = new Float32Array(profile.density * 3);
  for (let i = 0; i < profile.density; i += 1) {
    positions[i * 3] = ((i * 17) % 400) / 10 - 20;
    positions[i * 3 + 1] = ((i * 29) % 220) / 10;
    positions[i * 3 + 2] = ((i * 41) % 400) / 10 - 20;
    velocities[i * 3] = 0.15 + ((i * 7) % 10) / 30;
    velocities[i * 3 + 1] = -profile.speed * (0.7 + ((i * 11) % 10) / 25);
    velocities[i * 3 + 2] = 0.05 + ((i * 13) % 10) / 50;
  }

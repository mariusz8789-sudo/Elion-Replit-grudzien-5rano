...
export function buildSw4CitySpecification(options: Sw4EpidemiologyCityOptions): WorldSpecification {
  return {
    ...
    // Reuse the canonical opt-in geometry generator so this exact simulated
    // WorldGraph also contains renderable roads/buildings. The SEIR state still
    // lives only on the population entity created by EPIDEMIOLOGY_TEMPLATE.
    structuralDetail: {
      citySizeM: 320,
      districtCount: options.districtCount ?? 4,
      parcelsPerDistrict: options.buildingsPerDistrict ?? 3,
      maxFloors: 6,
      generateInteriors: false,
      generateNavigation: false,
      requiredBuildingTypes: ['HOSPITAL'],
    },

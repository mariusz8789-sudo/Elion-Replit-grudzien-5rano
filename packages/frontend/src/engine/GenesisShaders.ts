...
uniform float uTime; uniform float uPrompt; varying vec2 vUv;
...
uniform float uTime; uniform float uPrompt; attribute float aEnergy; attribute float aW; varying float vEnergy;
void main(){ vec3 p=position; float t=uTime*(0.16+aEnergy*0.08)+uPrompt*6.283; float x=p.x*cos(t)-aW*sin(t); float z=p.z*cos(t*.71)-aW*sin(t*.71); p.x=x+sin(t+p.z+uPrompt)*.16; p.y+=cos(t*.83+p.x+uPrompt)*.18; p.z=z; vec4 mv=modelViewMatrix*vec4(p,1.0); gl_PointSize=(0.7+aEnergy*1.5)*(70.0/max(1.0,-mv.z)); gl_Position=projectionMatrix*mv; vEnergy=aEnergy; }`;

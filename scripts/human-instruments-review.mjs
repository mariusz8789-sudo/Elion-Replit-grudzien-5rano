...
const out = process.env.CONTEXT_ONLY ? 'artifacts/human-visual-ceiling/context' : 'artifacts/human-visual-ceiling/instruments';
...
async function sample(page,name) {
  ...
  const data=await page.evaluate(() => {
    ...
    const ui=['.human-hero-heading','.human-hero-tools','.human-hero-path','.human-hero-action','.human-context','.human-inspector:not([hidden])','[data-testid="mobile-navigation"]'].filter(selector=>{const el=document.querySelector(selector);return el&&window.getComputedStyle(el).visibility!=='hidden';}).map(selector=>({selector,rect:rect(document.querySelector(selector))})).filter(x=>x.rect?.width&&x.rect?.height);
...
async function peek(page,name) {
 await page.getByTestId('human-subject-target').click();
 await page.getByTestId('human-context').waitFor();
 const layout=await page.evaluate(()=>{
  const card=document.querySelector('[data-testid="human-context"]').getBoundingClientRect();
  const b=JSON.parse(document.querySelector('[data-testid="human-subject-target"]').dataset.subjectBounds);
  const c=document.querySelector('[data-testid="sw-canvas"]').getBoundingClientRect();
  const subject={left:b.left+c.left,right:b.right+c.left,top:b.top+c.top,bottom:b.bottom+c.top};
  return {height:card.height, viewport:window.innerHeight, overlap:Math.min(card.right,subject.right)>Math.max(card.left,subject.left)&&Math.min(card.bottom,subject.bottom)>Math.max(card.top,subject.top),subject,card:card.toJSON()};
 });
 assert.ok(layout.height/layout.viewport<=.35,`${name}: peek covers >35%`);
 assert.equal(layout.overlap,false,`${name}: peek obscures subject ${JSON.stringify(layout)}`);
 await sample(page,name);
 await page.keyboard.press('Escape');assert.equal(await page.getByTestId('human-context').count(),0);
 await page.getByTestId('human-subject-target').click();await page.getByTestId('sw-canvas').click({position:{x:page.viewportSize().width/2,y:230}});assert.equal(await page.getByTestId('human-context').count(),0);
}
async function instruments(page,name) {
 await page.getByTestId('human-inspector-toggle').click();
 await page.getByRole('searchbox',{name:'Szukaj narządu'}).fill(await page.getByTestId('sw-explorer-organ-heart').textContent());
 assert.equal(await page.getByTestId('sw-explorer-organ-heart').isVisible(),true);
 await page.getByRole('searchbox',{name:'Szukaj narządu'}).fill('');
 await page.getByTestId('human-tab-microscope').click();
 for(const mag of [1,5,25,100,500,1000])assert.equal(await page.getByTestId(`sw-explorer-mag-${mag}`).isVisible(),true);
 await sample(page,`${name}-microscope`);
 await page.getByTestId('human-tab-section').click();
 for(const axis of ['sagittal','coronal','axial']){await page.getByTestId(`sw-explorer-axis-${axis}`).click();assert.equal(await page.getByTestId(`sw-explorer-axis-${axis}`).getAttribute('aria-pressed'),'true');}
 await page.getByTestId('sw-explorer-cut-position').fill('0.65');assert.equal(await page.getByTestId('human-depth-readout').textContent(),'65%');
 await page.getByTestId('sw-explorer-cut-flip').click();
 for(const mode of ['normal','translucent','xray','ghost']){await page.getByTestId(`sw-explorer-surface-${mode}`).click();assert.equal(await page.getByTestId(`sw-explorer-surface-${mode}`).getAttribute('aria-pressed'),'true');}
 await page.getByTestId('sw-explorer-isolate').click();assert.equal(await page.getByTestId('sw-explorer-isolate').getAttribute('aria-pressed'),'true');
 await page.getByTestId('sw-explorer-isolate').click();
 await page.getByTestId('sw-explorer-surface-normal').click();
 await sample(page,`${name}-section`);
 await page.getByTestId('sw-explorer-cut-toggle').click();
 await page.getByTestId('human-tab-research').click();
 for(const id of ['test','neuro','obrazowanie','histologia']){await page.getByTestId(`sw-quick-${id}`).scrollIntoViewIfNeeded();assert.equal(await page.getByTestId(`sw-quick-${id}`).isVisible(),true);}
 assert.equal(await page.getByTestId('sw-input').count(),1);
 await sample(page,`${name}-research`);
 await page.getByTestId('human-inspector-toggle').click();
}
try {
 if(process.env.CONTEXT_ONLY) {
  for(const [width,height] of [[1440,900],[375,812],[390,844],[430,932]]) {
   const {ctx,page}=await open({width,height});
   if(width>900){await page.getByTestId('human-subject-target').hover();await page.getByTestId('human-context').waitFor();await page.mouse.move(10,10);assert.equal(await page.getByTestId('human-context').count(),0);}
   await level(page,'organ');await peek(page,`organ-${width}-peek`);await ctx.close();
  }
 } else {
 let {ctx,page}=await open({width:1440,height:900});
 await sample(page,'desktop-establishing');
 await peek(page,'desktop-peek');await instruments(page,'desktop');
 ...
 for(const [width,height] of [[375,812],[390,844],[430,932]]){
  ...
  await peek(page,`mobile-${width}-peek`);await instruments(page,`mobile-${width}`);await ctx.close();
 }
 }
